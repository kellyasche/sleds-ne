...
act_curve <- tibble(
  ...
  pct = map_dbl(act_scores, function(s) {
    ...
    mean(predict(model3, newdata = d, type = "response"))
  })
)

no_act_data <- master.model.5
no_act_data$took.ACT <- "No"
no_act_data$ACTCompositeScore.filled <- 0
no_act_pct <- mean(predict(model3, newdata = no_act_data, type = "response"))
...
ggplot(act_curve, aes(x = score, y = pct)) +
  ...
  labs(
    ...
    y = "Predicted likelihood of sustained regional workforce participation",
    title = "Predicted Likelihood of Sustained Regional Workforce Participation\nby ACT Participation and Score among college non-completers",
    subtitle = "Simply taking the ACT is associated with higher odds of sustained regional\nemployment at lower scores; among test-takers, this advantage fades and reverses as\nscores climb higher." ) +
  theme_bar +
  theme(legend.position = "bottom",
        axis.title = element_text(size = 7))

ggsave(filename = "Charts/Draft 5/sustained-wf-region-act.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 5)

ggsave(filename = "Charts/Draft 5/sustained-wf-region-act.png", dpi = "print", width = 6, height = 5)
