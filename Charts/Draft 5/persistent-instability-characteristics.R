...
# NOTE: replace `model3_non_sustained` and `master.disc` with your actual
# fitted model object and dataset name for the fully-specified instability model
...
dumbbell_data <- tibble(
  ...
  baseline_pct = c(
    amp(model3_non_sustained, master.disc, "SpecialEdStatus", "0"),
    amp(model3_non_sustained, master.disc, "Gender", "M"),
    amp(model3_non_sustained, master.disc, "RaceEthnicity", "White"),
    amp(model3_non_sustained, master.disc, "RaceEthnicity", "White"),
    amp(model3_non_sustained, master.disc, "pseo.participant", "0"),
    amp(model3_non_sustained, master.disc, "took.ACT", "No")
  ),
  higher_pct = c(
    amp(model3_non_sustained, master.disc, "SpecialEdStatus", "1"),
    amp(model3_non_sustained, master.disc, "Gender", "F"),
    amp(model3_non_sustained, master.disc, "RaceEthnicity", "AI"),
    amp(model3_non_sustained, master.disc, "RaceEthnicity", "Black"),
    amp(model3_non_sustained, master.disc, "pseo.participant", "1"),
    amp(model3_non_sustained, master.disc, "took.ACT", "Yes")
  )
) %>%
  mutate(
...
ggplot(dumbbell_data, aes(y = characteristic)) +
  ...
  labs(
    x = "Predicted likelihood of persistent non-sustained employment",
    y = NULL,
    title = "Predicted Likelihood of Persistent Non-Sustained Employment by\nSelected Characteristics",
    subtitle = "Special Education status, Gender, American Indian and Black identity, PSEO\nparticipation, and ACT participation are all independently associated with higher\ninstability risk."
  ) +
  theme_bar +
  theme(legend.position = "bottom",
        axis.title = element_text(size = 7))

ggsave(filename = "Charts/Draft 5/persistent-instability-characteristics.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 5)

ggsave(filename = "Charts/Draft 5/persistent-instability-characteristics.png", dpi = "print", width = 6, height = 5)
