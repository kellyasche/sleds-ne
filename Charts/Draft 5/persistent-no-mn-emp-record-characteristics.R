...
# NOTE: replace `model3_no_record` with your fitted no-record model object

dumbbell_data <- tibble(
  characteristic = c(
    "English Learner Status\n(vs. Not English Learner)",
    "Hispanic\n(vs. White)",
    "Asian/Pacific Islander\n(vs. White)",
    "Black\n(vs. White)"
  ),
  baseline_pct = c(
    amp(model3_no_record, master.disc, "english.learner", "0"),
    amp(model3_no_record, master.disc, "RaceEthnicity", "White"),
    amp(model3_no_record, master.disc, "RaceEthnicity", "White"),
    amp(model3_no_record, master.disc, "RaceEthnicity", "White")
  ),
  higher_pct = c(
    amp(model3_no_record, master.disc, "english.learner", "1"),
    amp(model3_no_record, master.disc, "RaceEthnicity", "Hispanic"),
    amp(model3_no_record, master.disc, "RaceEthnicity", "Asian/PI"),
    amp(model3_no_record, master.disc, "RaceEthnicity", "Black")
  )
) %>%
  mutate(
...
ggplot(dumbbell_data, aes(y = characteristic)) +
  ...
  labs(
    ...
    y = NULL,
    title = "Predicted Likelihood of Persistent No-Record Status by Selected\nCharacteristics",
    subtitle = "English learner status, and Hispanic, Asian/Pacific Islander, and Black identity are all\nindependently associated with higher odds of persistently having no Minnesota\nemployment record."
  ) +
  theme_bar +
  theme(legend.position = "bottom",
        axis.title =  element_text(size = 7))

ggsave(filename = "Charts/Draft 5/persistent-no-mn-emp-record-characteristics.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 5)

ggsave(filename = "Charts/Draft 5/persistent-no-mn-emp-record-characteristics.png", dpi = "print", width = 6, height = 5)
