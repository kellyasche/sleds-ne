...
# ---- Compute both values for each characteristic ----

dumbbell_data <- tibble(
  characteristic = c("Regional college\n(vs. Elsewhere)",
                     "Associate or lower credential\n(vs. Bachelor's)"),
  baseline_pct = c(
    amp(model5_dv1b, master.model.dv1b, "ps.grad.location", "PS grad outside region"),
    amp(model5_dv1b, master.model.dv1b, "highest.cred.level", "Bachelor degree or higher")
  ),
  higher_pct = c(
    amp(model5_dv1b, master.model.dv1b, "ps.grad.location", "PS grad Region"),
    amp(model5_dv1b, master.model.dv1b, "highest.cred.level", "Associate or lesser credential")
  )
) %>%
  mutate(
...
ggplot(dumbbell_data, aes(y = characteristic)) +
  ...
  scale_color_manual(
    name = NULL,
    values = c("Without characteristic" = color_baseline, "With characteristic" = color_highlight),
  ...
  labs(
    ...
    y = NULL,
    title = "Predicted Likelihood of Sustained Regional Workforce Participation\nby Graduation Location and Credential Level",
    subtitle = "Graduation location shows a much larger relationship with regional workforce\nparticipation than credential level, though credential level still adds a real,\nindependent effect."
  ) +
  theme_bar +
  theme(legend.position = "bottom",
        axis.title.x = element_text(size = 6))

ggsave(filename = "Charts/Draft 5/ps-grad-location-sustained-wf-ne.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Draft 5/ps-grad-location-sustained-wf-ne.png", dpi = "print", width = 6, height = 4)
