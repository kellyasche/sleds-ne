...
dumbbell_data <- tibble(
  characteristic = c("Gender\n(Female vs. Male)",
                     ...
                     "PSEO Participation\n(vs. Non-Participant)"),
  baseline_pct = c(
    amp(model3_dv2, master.model.dv2, "Gender", "M"),
    amp(model3_dv2, master.model.dv2, "range.school", "Non-range school"),
    amp(model3_dv2, master.model.dv2, "pseo.participant", "0")
  ),
  higher_pct = c(
    amp(model3_dv2, master.model.dv2, "Gender", "F"),
    amp(model3_dv2, master.model.dv2, "range.school", "Range school"),
    amp(model3_dv2, master.model.dv2, "pseo.participant", "1")
  )
) %>%
  mutate(
...
ggplot(dumbbell_data, aes(y = characteristic)) +
  ...
  geom_text(aes(x = baseline_pct, label = percent(baseline_pct, accuracy = 1)),
            vjust = -1.6, size = 4, color = color_baseline) +
  geom_text(aes(x = higher_pct, label = percent(higher_pct, accuracy = 1)),
            vjust = -1.6, size = 4, color = color_highlight, fontface = "bold") +
  scale_color_manual(
    name = NULL,
  ...
  labs(
    ...
    subtitle = "Each characteristic was associated with a roughly 9 to 10 percentage point higher\nlikelihood of completion, after accounting for other differences between graduates."
  ) +
  theme_bar +
...
ggsave(filename = "Charts/Draft 5/ps-grad-gender-range-school-pseo.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Draft 5/ps-grad-gender-range-school-pseo.png", dpi = "print", width = 6, height = 4)
