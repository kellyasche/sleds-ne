...
ggplot(data = or_plot, aes(x = estimate, y = term)) +
  ...
  annotate(geom = "text",
           x = 1.66,
           y = 8.8,
           label = "67% higher odds than males",
           size = 3) +
  geom_point() +
  geom_errorbar(
  ...
  scale_y_discrete(labels = c("Avg unemployment rate", "MCA - R > 2", "MCA - S > 2", "MCA - M > 2", "Town/rural mix district", "Urban/town/rural mix district", "PSEO participant", "ACT > 20", "Female")) +
  theme_line +
  labs(
    x = "Odds of completing a postsecondary\ncredential",
    y = NULL,
    title = "Factors Associated with Postsecondary Credential\nCompletion",
    subtitle = "When controlling for other variables, female students show higher\nodds of completing a credential than male students."
  ) +
  theme(text = element_text(size = 14))

ggsave(filename = "Charts/Paper report 2/college-grad-female-odds.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report 2/college-grad-female-odds.png", dpi = "print", width = 6, height = 4.5)
