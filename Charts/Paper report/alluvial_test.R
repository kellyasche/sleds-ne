...
cte.order <- c("No CTE", "CTE Participant", "CTE Concentrator")

grad.year.5.order <- c("No MN employment", "Not meaningful", "Meaningful WF - MN", "Meaningful WF - NE")

data.sankey <- data %>%
  make_long(cte.achievement, grad.year.5, value = pct) %>%
  mutate(
    x = str_replace(x, "cte.achievement", "CTE"),
    x = str_replace(x, "grad.year.5", "5 years after\nhigh school"),
    x = fct_relevel(x, "CTE", "5 years after\nhigh school"),
    node      = fct_relevel(node, cte.order, grad.year.5.order),
    next_node = fct_relevel(next_node, grad.year.5.order)
  ) %>%
  mutate(flow_id = cumsum(!is.na(next_node))) %>%
  group_by(flow_id) %>%
  mutate(highlight = ifelse(next_node == "Meaningful WF - NE", "highlight", "base"),
         highlight = ifelse(node == "Meaningful WF - NE", "highlight", highlight),
         highlight = ifelse(any(next_node == "Meaningful WF - NE", na.rm = TRUE), "highlight", "base")) %>%
  ungroup()
