...
data <- original %>%
  ...
  group_by(ps.grad, economic.status) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(economic.status) %>%
  mutate(pct = n / sum(n)) %>%
  ungroup() %>%
  mutate(economic.status = ifelse(economic.status == 1, "Eligible", "Not eligible"),
...
ggplot(data = data, aes(economic.status, pct, fill = ps.grad, group = ps.grad)) +
...
ggsave(filename = "Charts/Paper report/economic-status-ps-grad.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report/economic-status-ps-grad.png", dpi = "print", width = 6, height = 4.5)
