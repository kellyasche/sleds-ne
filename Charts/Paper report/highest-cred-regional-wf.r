...
# Create chart ------------------------------------------------------------
ggplot(data = filter(data, grad.year.5 == "Meaningful WF - Northeast"), aes(highest.cred.level, pct, fill = highest.cred.level)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
  annotate(geom = "label",
           x = 1,
           y = .35,
           label = "2-year or less credential\nincreases participation in region",
           size = 3) +
  labs(x="", y = "", title = "Share of highest credential earned by meaningful workforce\nparticipation in the region 5-years after high school")+
  scale_y_continuous(labels=scales::percent) +
  scale_x_discrete(labels = c( "Associate degree or lesser credential" = "Associate degree\nor lesser", "Bachelor degree or higher" = "Bachelor degree\nor higher", "Did not graduate ps" = "Did not graduate\nps")) +
...
ggsave(filename = "Charts/Paper report/highest-cred-regional-wf.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report/highest-cred-regional-wf.png", dpi = "print", width = 6, height = 4.5)
