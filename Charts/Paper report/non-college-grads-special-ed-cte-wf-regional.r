...
ggplot(data = filter(data, grad.year.5 == "Meaningful WF - Northeast"), aes(cte.achievement, pct, fill = cte.achievement)) +
  ...
  annotate(geom = "label",
           ...
           size = 2) +
  labs(x="", y = "",
       title = "Non-college grads requiring special education with\nmeaningful workforce participation in the region by\nCTE engagement")+
...
ggsave(filename = "Charts/Paper report/non-college-grads-special-ed-cte-wf-regional.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Paper report/non-college-grads-special-ed-cte-wf-regional.png", dpi = "print", width = 6, height = 4)
