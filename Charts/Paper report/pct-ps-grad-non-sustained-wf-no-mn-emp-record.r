...
data <- original %>%
  ...
  mutate(pct = n / sum(n)) %>%
  ungroup() %>%
  mutate(ps.grad = ifelse(ps.grad == "Y", "PS Grad", "Non PS Grad"),
         ps.grad = fct_relevel(ps.grad, "PS Grad", "Non PS Grad"),
...
ggplot(data, aes(grad.year.5, pct, fill = ps.grad, group = ps.grad)) +
  geom_col(position = "dodge", alpha = ifelse(data$grad.year.5 %in% c("Non-sustained WF", "No MN emp record"), 1, .5)) +
  geom_label(alpha = ifelse(data$grad.year.5 %in% c("Non-sustained WF", "No MN emp record"), 1, .5), aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 3) +
  labs(x="", 
       y = "", 
       title = "Risk of weak workforce attachment is concentrated among\nnon-college graduates", 
       subtitle = "Non-college graduates are substantially more likely to experience non-sustained\nworkforce participation or lack a Minnesota employment record")+
...
ggsave(filename = "Charts/Paper report/pct-ps-grad-non-sustained-wf-no-mn-emp-record.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report/pct-ps-grad-non-sustained-wf-no-mn-emp-record.png", dpi = "print", width = 6, height = 4.5)
