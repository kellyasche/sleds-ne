...
ggplot(data = filter(data, states %in% c("No MN emp record", "No meaningful WF")), aes(grad.year, share, fill = states, group = states)) +
  facet_wrap(~states, ncol = 2) +
  geom_col(position = "dodge") +
  geom_label(data = filter(data, states %in% c("No MN emp record", "No meaningful WF")), aes(label = percent(share, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
  labs(x="Years after graduating high school", y = "", title = "Workforce participation categories", subtitle = "Graduates that are disconnected from the Minnesota workforce make up nearly half\nof the high school graduates.")+
...
ggsave(filename = "Charts/Paper report/wf-disconnected.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Paper report/wf-disconnected.png", dpi = "print", width = 6, height = 4)
