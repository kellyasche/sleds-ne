...
data <- original %>%
  ...
  mutate(states = ifelse(states == "No MN emp record, not attending ps" & ps.grad == "No", "No MN emp record, non-college grad", states)) %>%
  ...
  mutate(grad.year = fct_relevel(grad.year, "grad.year.0", "grad.year.1", "grad.year.2", "grad.year.3", "grad.year.4", "grad.year.5", "grad.year.6", "grad.year.7", "grad.year.8", "grad.year.9", "grad.year.10", "grad.year.11", "grad.year.12", "grad.year.13", "grad.year.14", "grad.year.15"),
         states = ifelse(states %in% c("Meaningful emp EDR", "Meaningful emp County"), "Meaningful WF - NE", states),
         states = str_replace(states, "Not meaningful, not attending ps", "Not meaningful WF"),
         states = str_replace(states, "Meaningful emp MN", "Meaningful WF - MN"),
         states = str_replace(states, "No MN emp record, not attending ps", "No MN emp record - college-grad"),
         states = fct_relevel(states, "Meaningful WF - NE", "Meaningful WF - MN", "Not meaningful WF", "No MN emp record - college-grad", "No MN emp record, non-college grad", "Attending ps")) %>%
  ...
  ungroup() %>%
  group_by(states) %>%
  mutate(alpha = ifelse(states == "Meaningful WF - NE", 1, .6)) %>%
  ungroup() %>%
...
ggplot(data = filter(data, states == "No MN emp record, non-college grad"), aes(year.after.hs, pct)) +
  geom_col(position = "dodge", fill = "#fee090") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), size = 3) +
  labs(x="", 
       y = "",
       title = "Non-college graduates with no Minnesota employment record over time",
       subtitle = "The share of non-college graduates without a Minnesota employment record increases\nsteadily as cohorts age.")+
  scale_y_continuous(labels=scales::percent) +
  theme_bar+
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 25, hjust = .9))

ggsave(filename = "Charts/Paper report/pct-no-mn-emp-record-noncollege-grad-wf.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 5)

ggsave(filename = "Charts/Paper report/pct-no-mn-emp-record-noncollege-grad-wf.png", dpi = "print", width = 6.5, height = 5)
