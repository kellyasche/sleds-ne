...
# Create chart ------------------------------------------------------------
ggplot(data = filter(data, grad.year.5 == "Meaningful WF - Northeast"), aes(ps.grad.location, pct, fill = ps.grad.location)) +
  ...
  annotate(geom = "label",
           x = 3,
           y = .4,
           label = "Graduating college in region\nincreasess participation in region",
           size = 3) +
  labs(x="", y = "", title = "College institution location as percent of meaningful\nworkforce participation 5 years after graduating high school")+
...
ggsave(filename = "Charts/Paper report/college-location-regional-wf.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report/college-location-regional-wf.png", dpi = "print", width = 6, height = 4.5)
