...
data <- original %>%
  ...
  ungroup() %>%
  mutate(pct = n / sum(n),
         disconnected.cat = fct_relevel(disconnected.cat, "Persistently disconnected", "Broadly disconnected", "Intermittently disconnected", "Consistently attached"))
...
ggplot(data, aes(disconnected.cat, pct, fill = disconnected.cat)) +
  ...
  labs(x="", 
       y = "", 
       title = "Distribution of non-college graduates by pattern of workforce disconnection over time",
       subtitle = "Weak workforce connection often persists among non-college graduates.")+
  scale_y_continuous(labels=scales::percent) +
  scale_x_discrete(labels = c("Persistently\ndisconnected", "Braodly\ndisconnected", "Intermittently\ndisconnected", "Consistently\nattached")) +
  theme_bar+
  scale_fill_manual(values = brewer.pal(n = 5, "BrBG"),
                    guide = guide_legend(ncol = 2)) +
  theme(legend.position = "none")

ggsave(filename = "Charts/Paper report/pct-non-college-grads-disconnection-cat.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 4)

ggsave(filename = "Charts/Paper report/pct-non-college-grads-disconnection-cat.png", dpi = "print", width = 6.5, height = 4)
