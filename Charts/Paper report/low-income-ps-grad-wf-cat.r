...
data.ps.grad <- original %>%
  ...
  mutate(economic.status = ifelse(economic.status == 1, "Eligible - free or reduced lunch", "Not eligible - free or reduced lunch")) %>%
  group_by(grad.year.5, economic.status) %>%
  summarize(n = n()) %>%
  ungroup() %>%
...
ps.grad.plot <- ggplot(data = filter(data.ps.grad, economic.status == "Eligible - free or reduced lunch"), aes(grad.year.5, pct, fill = grad.year.5)) +
...
not.ps.grad.plot <- ggplot(data = filter(data.not.ps.grad, economic.status == "Eligible - free or reduced lunch"), aes(grad.year.5, pct, fill = grad.year.5)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 3) +
  annotate(geom = "label",
           x = 3.5,
...
title <- ggdraw() +
  draw_label("Lower income students - college and non-college grads by\nworkforce participation category",
             fontfamily = "Avenir",
             fontface = "bold",
             x = 0,
             hjust = 0,
...
ggsave(filename = "Charts/Paper report/low-income-ps-grad-wf-cat.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 5)

ggsave(filename = "Charts/Paper report/low-income-ps-grad-wf-cat.png", dpi = "print", width = 6.5, height = 5)
