...
data <- original %>%
  ...
  group_by(ps.grad.location, grad.year.5) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(ps.grad.location) %>%
  mutate(pct = n / sum(n)) %>%
  ungroup() %>%
  mutate(grad.year.5 = str_replace(grad.year.5, "Not meaningful, not attending ps", "Non-sustained WF"),
         ...
         ps.grad.location = fct_relevel(ps.grad.location, "PS grad Region", "PS grad MN", "PS grad outside MN"))
...
df_lodes <- data %>%
  ...
  to_lodes_form(
    axes = c("ps.grad.location", "grad.year.5"),
    key = "axis",
    value = "stratum",
    id = "flow_id"
  ) %>%
  ...
  group_by(flow_id) %>%
  mutate(
    ps.grad.location = first(stratum[axis == "ps.grad.location"])
  ) %>%
  ungroup()


labels <- df_lodes %>%
  mutate(add.loc.y = lead(pct)) %>%
  filter(axis == "ps.grad.location", highlight == "highlight") %>%
  arrange(ps.grad.location) %>%
  select(stratum, pct, add.loc.y) %>%
  mutate(half.pct = pct / 2,
         cumsum = rev(seq(n())),
         yloc = cumsum - add.loc.y - half.pct,
         highlight = "highlight") %>%
  select(-pct)
...
ggplot(
  ...
  aes(
    ...
    alluvium = flow_id,
    y = pct,
    fill = ps.grad.location,     # left-side colors
  ...
  geom_text(
    stat = "stratum",
    label = c(rev(ps.grad.location.order.plot), rev(grad.year.5.order.plot)),
    size = 3
  ) +
  ...
  scale_x_discrete(
    limits = c("ps.grad.location", "middle", "grad.year.5"),
    labels = c("Post-secondary\ncompletion\nlocation", "", "5 years after\nhigh school")
  ) +
  labs(x = "", 
       y = "",
       title = "Workforce participation outcomes five year after high school, by location\nof postsecondary graduation",
       subtitle = "Graduation location is strongly associated with workforce attachment outside the region") +
  theme_all +
  theme(legend.position = "none",
        text = element_text(size = 10))

ggsave(filename = "Charts/Paper report/sankey-ps-grad-location-meaningfully-participating-wf-mn.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 4.5)

ggsave(filename = "Charts/Paper report/sankey-ps-grad-location-meaningfully-participating-wf-mn.png", dpi = "print", width = 6.5, height = 4.5)
