...
df_lodes <- data %>%
  ...
  mutate(
    highlight = if_else(
      any(stratum == "PS grad outside MN"),
      "highlight",
      "base"
    )
  ) %>%
...
labels <- df_lodes %>%
  ...
  mutate(half.pct = pct / 2,
         cumsum = rev(seq(n())),
         remainder = 1 - pct,
         yloc = cumsum - remainder - half.pct,
         highlight = "highlight") %>%
  select(-pct)
...
ggplot(
  ...
  labs(x = "", 
       y = "",
       title = "Completed college outside Minnesota, by CTE Achievement",
       subtitle = "The less engaged an individual is in CTE, the higher the proportion of individuals that\ncomplete college outside MN") +
  theme_all +
  theme(legend.position = "none")


ggsave(filename = "Charts/Paper report/sankey-cte-acheivement-outside-mn-college.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 4.5)

ggsave(filename = "Charts/Paper report/sankey-cte-acheivement-outside-mn-college.png", dpi = "print", width = 6.5, height = 4.5)
