...
ggplot(data = filter(data, grad.year.5 == "Meaningful WF - Northeast"), aes(cte.achievement, pct, fill = cte.achievement)) +
  ...
  annotate(geom = "label",
           ...
           y = .11,
           angle = -25,
           label = "CTE engagement significantly increases\nmeaningful workforce participation in region\namong non-college, low-income individuals",
           color = "black",
           fill = "white",
           size = 2) +
  labs(x="", y = "",
       title = "Non-college grads eligible for free or reduced lunch\nwith meaningful workforce participation in the region\nby CTE engagement")+
...
ggsave(filename = "Charts/Paper report/non-college-grads-low-income-cte-wf-regional.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Paper report/non-college-grads-low-income-cte-wf-regional.png", dpi = "print", width = 6, height = 4)
