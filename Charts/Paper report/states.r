...
ggplot(data, aes(grad.year, share, fill = states, group = states)) +
  ...
  geom_label(data = filter(data, grad.year %in% c(1,5,10, 15)), aes(label = percent(share, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
  labs(x="", y = "", title = "Pathways taken by share of graduates X years\nafter graduating high school")+
...
ggsave(filename = "Charts/Paper report/states.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 5)
