...
data
# Create chart ------------------------------------------------------------
ggplot(data = filter(data, ps.grad.location == "PS grad outside MN" ), aes(grad.year.5, pct, fill = grad.year.5)) +
  ...
  labs(x="", 
       y = "", 
       title = "Share that graduated from a college outside Minnesota\nby workforce category 5-years after high school")+
...
ggsave(filename = "Charts/Paper report/college-location-outside-mn-wf.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 4.5)

ggsave(filename = "Charts/Paper report/college-location-outside-mn-wf.png", dpi = "print", width = 6.5, height = 4.5)
