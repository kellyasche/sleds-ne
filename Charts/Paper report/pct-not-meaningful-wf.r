...
ggplot(data = filter(data, states == "Non-sustained WF"), aes(year.after.hs, pct)) +
  geom_col(position = "dodge", fill = "#e0f3f8") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), size = 3) +
  labs(x="", 
       y = "",
       title = "Non-sustained workforce participation among non-college graduates over\ntime",
       subtitle = "Non-sustained workforce attachment is common immediately after high school and remains\nprevalent many years later")+
...
ggsave(filename = "Charts/Paper report/pct-non-college-grads-non-sustained-wf.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 5)

ggsave(filename = "Charts/Paper report/pct-non-college-grads-non-sustained-wf.png", dpi = "print", width = 6.5, height = 5)
