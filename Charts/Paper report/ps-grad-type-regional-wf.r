...
data <- original %>%
  ...
  filter(!ps.grad.InstitutionSector %in% c("Attending ps")) %>%
  mutate(ps.grad.InstitutionSector = ifelse(ps.grad.InstitutionSector %in% c("Did not grad", "Never attended ps"), "Did not graduate college", ps.grad.InstitutionSector),
  ...
  mutate(grad.year.5 = str_replace(grad.year.5, "No MN emp record, not attending ps", "No MN emp record"),
         ...
         ps.grad.InstitutionSector = fct_relevel(ps.grad.InstitutionSector, "2-year public", "4-year public", "Other type of college", "Did not graduate college"))

levels(data$ps.grad.InstitutionSector)
# Create chart ------------------------------------------------------------
ggplot(data = filter(data, grad.year.5 == "Meaningful WF - Northeast"), aes(ps.grad.InstitutionSector, pct, fill = ps.grad.InstitutionSector)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
  annotate(geom = "label",
           x = 1.2,
           y = .35,
           label = "Graduating from 2-year colleges\nincreases participation in region",
           size = 3) +
  labs(x="", y = "", title = "Share of college institution type by meaningful workforce\nparticipation in the region 5-years after high school")+
  scale_y_continuous(labels=scales::percent) +
  scale_x_discrete(labels = c("2-year public" = "2-year public", "4-year public" = "4-year public", "Other type of college" = "Other type\nof college", "Did not graduate college" = "Did not\ngraduate college")) +
...
ggsave(filename = "Charts/Paper report/ps-grad-type-regional-wf.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report/ps-grad-type-regional-wf.png", dpi = "print", width = 6, height = 4.5)
