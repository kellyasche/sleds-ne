...
data.ps.grad.InstitutionSector <- original %>%
  ...
  mutate(grad.year.5 = str_replace(grad.year.5, "No MN emp record, not attending ps", "No MN emp record"),
         ...
         ps.grad.InstitutionSector = str_replace(ps.grad.InstitutionSector, "PS grad Region", "PS grad NE")) 

data.highest.cred.level <- original %>%
  select(grad.year.5, highest.cred.level) %>%
  filter(grad.year.5 != "After 2023") %>%
  filter(grad.year.5 != "Attending ps") %>%
  filter(!highest.cred.level %in% c("Attending ps")) %>% 
  mutate(highest.cred.level = ifelse(highest.cred.level %in% c("Associate degree", "Less than Associate Degree"), "Associate degree or lesser", highest.cred.level)) %>%
  ...
  mutate(grad.year.5 = str_replace(grad.year.5, "No MN emp record, not attending ps", "No MN emp record"),
         ...
         grad.year.5 = str_replace(grad.year.5, "Meaningful WF - Northeast", "Meaningful WF - NE"),
         grad.year.5 = fct_relevel(grad.year.5, "Meaningful WF - NE", "Meaningful WF - MN", "Not meaningful", "No MN emp record"),
         highest.cred.level = str_replace(highest.cred.level, "PS grad Region", "PS grad NE")) 

# Create chart ------------------------------------------------------------
ps.grad.InstitutionSector.plot <- ggplot(data = filter(data.ps.grad.InstitutionSector, ps.grad.InstitutionSector == "4-year public"), aes(grad.year.5, pct, fill = grad.year.5)) +
  ...
  labs(x="", 
       y = "", 
       subtitle = "Graduated from 4-year public college")+
...
ps.grad.InstitutionSector.plot

highest.cred.level.plot <- ggplot(data = filter(data.highest.cred.level, highest.cred.level == "Bachelor degree or higher"), aes(grad.year.5, pct, fill = grad.year.5)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 3) +
  annotate(geom = "label",
           x = 2, 
  ...
  labs(x="", 
       y = "", 
       subtitle = "Earned Bachelor Degree")+
  scale_y_continuous(labels=scales::percent) +
  scale_x_discrete(labels = c("Meaningful\nWF NE", "Meaningful\nWF MN", "Not\nmeaningful", "No MN\nEmp record")) +
  scale_fill_manual(values = c("#1a9641", "#0571b0", "#a6611a", "grey")) +
  theme_bar+
...
highest.cred.level.plot

title <- ggdraw() +
  draw_label("Share of 4-year college grads and Bachelor degree earners by\nworkforce participation category",
             x = 0,
             hjust = 0,
             fontface = "bold",
             size = 14,
...
top <- plot_grid(ps.grad.InstitutionSector.plot, highest.cred.level.plot, ncol = 2, rel_widths = c(1,1))

top

plot_grid(title, top, ncol = 1, rel_heights = c(.1, .9))

ggsave(filename = "Charts/Paper report/4-year-public-bachelor-degree-wf-states.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 4.5)

ggsave(filename = "Charts/Paper report/4-year-public-bachelor-degree-wf-states.png", dpi = "print", width = 6.5, height = 4.5)
