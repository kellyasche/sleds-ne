...
data <- original %>%
  ...
  mutate(grad.year.5 = str_replace(grad.year.5, "No MN emp record, not attending ps", "No MN employment"),
         ...
         cte.achievement = str_replace(cte.achievement, "CTE Concentrator or Completor", "CTE Concentrator"))
...
write_csv(df_lodes, "Charts/df_lodes.csv")
...
ggplot(
  ...
  geom_text(
    stat = "stratum",
    label = c(rev(cte.order.plot), rev(grad.year.5.order.plot)),
    size = 3
  ) +
  ...
  labs(x = "", 
       y = "",
       title = "Proportion of CTE achievement categories that have meaningful\nworkforce participation in NE",
       subtitle = "The more engaged an individual is in CTE, the higher the likelihood they will have\nmeaningful WF participation in Northeast") +
  theme_all +
  theme(legend.position = "none")

ggsave(filename = "Charts/Paper report/prop-cte-acheivement-meaningful-wf-ne.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report/prop-cte-acheivement-meaningful-wf-ne.png", dpi = "print", width = 6, height = 4.5)
