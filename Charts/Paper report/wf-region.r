...
ggplot(data = filter(data, states == "Meaningful WF - Northeast"), aes(grad.year, share, fill = states, group = states)) +
  geom_col(position = "dodge") +
  geom_label(data = filter(data, states == "Meaningful WF - Northeast"), aes(label = percent(share, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
  labs(x="Years after graduating high school", y = "", title = "Meaningful workforce participation in Northeast", subtitle = "Meaningful workforce participation in the region peaks at nearly 30% of high school\ngraduates")+
...
ggsave(filename = "Charts/Paper report/wf-region.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Paper report/wf-region.png", dpi = "print", width = 6, height = 4)
