...
data
# Create chart ------------------------------------------------------------
ggplot(data = filter(data, ps.grad.location == "PS grad MN" ), aes(grad.year.5, pct, fill = grad.year.5)) +
  ...
  labs(x="", 
       y = "", 
       title = "Share of college graduates in Minnesota but outside the region by\nworkforce participation categories 5-years after high school")+
...
ggsave(filename = "Charts/Paper report/college-grad-mn-wf.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 4.5)

ggsave(filename = "Charts/Paper report/college-grad-mn-wf.png", dpi = "print", width = 6.5, height = 4.5)
