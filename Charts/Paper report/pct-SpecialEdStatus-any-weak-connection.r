...
ggplot(data = filter(data, grad.year.5 == "Any weak connection"), aes(SpecialEdStatus, pct, fill = c("#018571", "#a6611a"))) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
  labs(x="", 
       y = "", 
       title = "Weak workforce connection is more common among\nnon-college graduates with special education identification", 
       subtitle = "Share of non-college graduates with any weak connection to workforce pathways,\nfive years after high school")+
...
ggsave(filename = "Charts/Paper report/pct-SpecialEdStatus-any-weak-connection.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Paper report/pct-SpecialEdStatus-any-weak-connection.png", dpi = "print", width = 6, height = 4)
