...
data <- original %>%
  ...
  filter(!ps.grad %in% c("Attending ps", "After 2023")) %>%
  mutate(SpecialEdStatus = ifelse(SpecialEdStatus == 1, "Special ed", "Not special ed")) %>%
  group_by(ps.grad, SpecialEdStatus) %>%
  summarize(n = n()) %>%
  ungroup() %>%
...
ggplot(data = data, aes(SpecialEdStatus, pct, fill = ps.grad, group = ps.grad)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 3) +
  labs(x="", y = "", title = "Free or reduced lunch eligibilty by college completion", subtitle = "Lower income students significantly more likely to not graduate\nfrom college")+
  scale_y_continuous(labels=scales::percent) +
  scale_x_discrete(labels = c("Free or reduced\nlunch eligible", "Not eliglble for free\nor reduced lunch")) +
  theme_bar+
...
ggsave(filename = "Charts/Paper report/SpecialEdStatus-ps-grad.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report/SpecialEdStatus-ps-grad.png", dpi = "print", width = 6, height = 4.5)
