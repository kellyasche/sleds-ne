...
data <- original %>%
  ...
  left_join(ps.grads, by = "PersonID") %>%
  mutate(states = ifelse(states == "No MN emp record" & ps.grad == "No", "Non-college grad - No MN emp record", as.character(states))) %>%
...
ggplot(data = filter(data, states == "Non-college grad - No MN emp record"), aes(grad.year, pct)) +
  ...
  labs(x="", 
       y = "",
       title = "Share of non-college graduates that have no Minnesota employment\nrecord",
       subtitle = "A consierable number of non-college grads don't hae a Minnesota employment record years\nafter high school.")+
...
ggsave(filename = "Charts/Paper report/pct-no-mn-emp-record.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 5)

ggsave(filename = "Charts/Paper report/pct-no-mn-emp-record.png", dpi = "print", width = 6.5, height = 5)
