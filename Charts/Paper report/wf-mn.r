...
ggplot(data = filter(data, states == "Meaningful WF - MN"), aes(grad.year, share, fill = states, group = states)) +
  geom_col(position = "dodge") +
  geom_label(data = filter(data, states == "Meaningful WF - MN"), aes(label = percent(share, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
  labs(x="Years after graduating high school", y = "", title = "Meaningful workforce participation in Minnesota", subtitle = "Meaningful workforce participation in the state peaks at nearly 20% of high school\ngraduates")+
...
ggsave(filename = "Charts/Paper report/wf-mn.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Paper report/wf-mn.png", dpi = "print", width = 6, height = 4)
