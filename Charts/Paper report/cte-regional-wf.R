...
ggplot(data = filter(data, grad.year.5 == "Meaningful WF - Northeast"), aes(cte.achievement, pct, fill = cte.achievement)) +
  ...
  annotate(geom = "label",
           x = 1.5,
           y = .2,
           label = "CTE engagement increases \nparticipation in region",
           size = 3) +
  labs(x="", y = "", title = "Share of CTE Achievement with meaningful workforce\nparticipation in the region 5 years after graduating\nhigh school")+
  scale_y_continuous(labels=scales::percent) +
  scale_x_discrete(labels = c("CTE Concentrator\nCompletor", "CTE\nParticipant", "No CTE")) +
  theme_bar+
...
ggsave(filename = "Charts/Paper report/cte-regional-wf.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Paper report/cte-regional-wf.png", dpi = "print", width = 6, height = 4)
