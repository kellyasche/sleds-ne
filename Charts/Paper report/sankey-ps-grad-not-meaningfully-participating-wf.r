...
data <- original %>%
  ...
  filter(SpecialEdStatus != "Attending ps",
         SpecialEdStatus != "Did not graduate ps") %>%
  group_by(SpecialEdStatus, grad.year.5) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(SpecialEdStatus) %>%
  mutate(pct = n / sum(n)) %>%
  ungroup() %>%
  mutate(SpecialEdStatus = ifelse(SpecialEdStatus == 1, "Special Education", "No Special Education"))

SpecialEdStatus.order <- c("Special Education", "No Special Education")

grad.year.5.order <- c("Sustained WF - NE", "Sustained WF - MN", "Non-sustained WF", "No MN emp record")

SpecialEdStatus.order.plot <- c("Special\nEducation", "No\nSpecial\nEducation")
...
df_lodes <- data %>%
  mutate(
    SpecialEdStatus = factor(SpecialEdStatus, levels = SpecialEdStatus.order),
    grad.year.5 = factor(grad.year.5, levels = grad.year.5.order)
  ) %>%
  to_lodes_form(
    axes = c("SpecialEdStatus", "grad.year.5"),
    key = "axis",
    value = "stratum",
    id = "flow_id"
  ) %>%
  ...
  group_by(flow_id) %>%
  mutate(
    SpecialEdStatus = first(stratum[axis == "SpecialEdStatus"])
  ) %>%
  ungroup()
...
labels <- df_lodes %>%
  filter(axis == "SpecialEdStatus", highlight == "highlight") %>%
  arrange(SpecialEdStatus) %>%
  select(flow_id, stratum, pct) %>%
  mutate(cumsum = ifelse(stratum == "Special Education", 1, 0)) %>%
  group_by(stratum) %>%
  mutate(cumpct = rev(cumsum(pct))) %>%
  ungroup() %>%
  mutate(half.pct = pct / 2,
         yloc = ifelse(flow_id %in% c(6,2), cumsum + half.pct, cumsum + cumpct - half.pct),
         highlight = "highlight") %>%
  select(-pct)
...
ggplot(
  ...
  aes(
    ...
    alluvium = flow_id,
    y = pct,
    fill = SpecialEdStatus,     # left-side colors
  ...
  geom_text(
    stat = "stratum",
    label = c(rev(SpecialEdStatus.order.plot), rev(grad.year.5.order.plot)),
    size = 3
  ) +
  ...
  scale_x_discrete(
    limits = c("SpecialEdStatus", "middle", "grad.year.5"),
    labels = c("Special\nEducation\nStatus", "", "5 years after\nhigh school")
  ) +
  labs(x = "", 
       y = "",
       title = "",
...
ggsave(filename = "Charts/Paper report/sankey-SpecialEdStatus-non-sustained-no-mn-emp-record-wf.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report/sankey-SpecialEdStatus-non-sustained-no-mn-emp-record-wf.png", dpi = "print", width = 6, height = 4.5)
