...
data.ACTCompositeScore <- original %>%
  ...
  group_by(ACTCompositeScore.cat, ps.grad.location) %>%
  summarize(n = n()) %>%
  ungroup() %>%
...
data.pseo <- original %>%
  ...
  group_by(pseo.participant, ps.grad.location) %>%
  summarize(n = n()) %>%
  ungroup() %>%
...
data.mca.m <- original %>%
  ...
  filter(ps.grad.location != "Attending ps") %>%
  filter(!is.na(MCA.M)) %>%
  mutate(MCA.M.cat = ifelse(MCA.M == 1, "Below proficient",
                            ifelse(MCA.M == 2, "Approaching proficient",
                                   ifelse(MCA.M == 3, "Proficient", "Exceeds proficient")))) %>%
  group_by(MCA.M.cat, ps.grad.location) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(MCA.M.cat) %>%
  mutate(pct = n / sum(n)) %>%
  ungroup() %>%
  mutate(MCA.M.cat = fct_relevel(MCA.M.cat, "Below proficient", "Approaching proficient", "Proficient", "Exceeds proficient"))


data.ACTCompositeScore$ACTCompositeScore.cat

# Create chart ------------------------------------------------------------
plot.took.ACT <- ggplot(data = filter(data.act, ps.grad.location == "PS grad outside MN"), aes(took.ACT, pct, fill = took.ACT)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
  geom_label(aes(x = 2, y = .07, label = "Significantly\nhigher"), color = "white", fill = "transparent") +
  labs(x="", 
       y = "", 
       subtitle = "Took ACT") +
...
plot.ACTCompositeScore.cat <- ggplot(data = filter(data.ACTCompositeScore, ps.grad.location == "PS grad outside MN"), aes(ACTCompositeScore.cat, pct, fill = ACTCompositeScore.cat)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
...
plot.pseo <- ggplot(data = filter(data.pseo, ps.grad.location == "PS grad outside MN"), aes(pseo.participant, pct, fill = pseo.participant)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
  annotate(geom = "label",
           x = 2,
...
plot.mca.m <- ggplot(data = filter(data.pseo, ps.grad.location == "PS grad MN"), aes(pseo.participant, pct, fill = pseo.participant)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 4) +
  annotate(geom = "label",
           x = 2,
...
title <- ggdraw() +
  draw_label("Proportion that graduated from a college outside Minnesota",
             fontface = "bold",
             x = 0,
             hjust = 0,
             fontfamily = "Avenir",
...
ggsave(filename = "Charts/Paper report/college-prep-ps-grad-outside-MN.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 5)

ggsave(filename = "Charts/Paper report/college-prep-ps-grad-outside-MN.png", dpi = "print", width = 6, height = 5)
