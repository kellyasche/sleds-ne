...
data <- original %>%
  ...
  filter(!ps.grad.location %in% c("Attending ps", "Did not graduate ps", "Never attended ps")) %>%
...
data
# Create chart ------------------------------------------------------------
ps.grad.mn.plot <- ggplot(data = filter(data, ps.grad.location == "PS grad MN" ), aes(grad.year.5, pct, fill = grad.year.5)) +
  ...
  labs(x="", 
       y = "", 
       subtitle = "In Minnesota, outside Northeast")+
...
ps.grad.outside.mn.plot <- ggplot(data = filter(data, ps.grad.location == "PS grad outside MN" ), aes(grad.year.5, pct, fill = grad.year.5)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "white", size = 3) +
  annotate(geom = "label",
           x = 4,
...
ps.grad.outside.mn.plot

top <- plot_grid(ps.grad.mn.plot, ps.grad.outside.mn.plot, ncol = 2, rel_widths = c(1,1))

title <- ggdraw() +
  draw_label("College graduate location by workforce participation category\n5-years after high school",
             fontface = "bold",
             x = 0, 
             hjust = 0,
             fontfamily = "Avenir",
...
plot_grid(title, top, ncol = 1, rel_heights = c(.1, 1))


ggsave(filename = "Charts/Paper report/college-location-outside-region-wf.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 5)

ggsave(filename = "Charts/Paper report/college-location-outside-region-wf.png", dpi = "print", width = 6.5, height = 5)
