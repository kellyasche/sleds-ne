...
data <- original %>%
  ...
  group_by(MCA.M, ps.grad.location) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(MCA.M) %>%
  mutate(pct = n / sum(n)) %>%
...
# Create chart ------------------------------------------------------------
ggplot(data = filter(data, ps.grad.location == "PS grad MN"), aes(MCA.M, pct, fill = MCA.M)) +
  ...
  labs(x="", 
       y = "", 
       title = "Share of MCA-Math proficiency scores by graduating from a\ncollege located in MN but outside the region") +
...
ggsave(filename = "Charts/Paper report/mca-m-ps-grad-MN.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report/mca-m-ps-grad-MN.png", dpi = "print", width = 6, height = 4.5)
