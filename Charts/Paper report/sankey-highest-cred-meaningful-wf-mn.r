...
data <- original %>%
  ...
  mutate(highest.cred.level = ifelse(highest.cred.level %in% c("Less than Associate Degree", "Associate degree"), "Associate degree or certificate", highest.cred.level)) %>%
...
highest.cred.level.order <- c("Associate degree or certificate", "Bachelor degree or higher", "Did not graduate ps")

grad.year.5.order <- c("Sustained WF - NE", "Sustained WF - MN", "Non-sustained WF", "No MN emp record")

highest.cred.level.order.plot <- c("Associate\ndegree or\ncertificate", "Bachelor\ndegree\nor higher", "Did not\ngraduate ps")
...
ggplot(
  ...
  scale_x_discrete(
    ...
    labels = c("Highest\ncredential\nearned", "", "5 years after\nhigh school")
  ) +
  labs(x = "", 
       y = "",
       title = "Workforce participation outcomes 5 years after high school, by highest\ncredential earned",
       subtitle = "Attained a Bachelor Degree or higher is strongly associated with Minnesota workforce\nattachment") +
  theme_all +
  theme(legend.position = "none")

ggsave(filename = "Charts/Paper report/sankey-highest-cred-meaningful-wf-mn.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 4.5)

ggsave(filename = "Charts/Paper report/sankey-highest-cred-meaningful-wf-mn.png", dpi = "print", width = 6.5, height = 4.5)
