...
data <- original %>%
  ...
  filter(!is.na(MCA.M)) %>%
  mutate(MCA.M = ifelse(MCA.M == 1, "Below proficient",
                        ifelse(MCA.M == 2, "Approaching proficient",
                               ifelse(MCA.M == 3, "Proficient",
                                      ifelse(MCA.M == 4, "Exceeds proficient", MCA.M)))),
         MCA.M = fct_relevel(MCA.M, "Below proficient", "Approaching proficient", "Proficient", "Exceeds proficient")) %>%
  group_by(MCA.M, grad.year.5) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(MCA.M) %>%
  mutate(pct = n / sum(n)) %>%
...
# Create chart ------------------------------------------------------------
ggplot(data = filter(data, grad.year.5 == "Meaningful WF - MN"), aes(MCA.M, pct, fill = MCA.M)) +
  ...
  labs(x="", 
       y = "", 
       title = "Share of MCA-Math proficiency scores in meaningful\nworkforce participation outside NE 5-years after high school",
       subtitle = "Higher MCA-Math scores increases the likelihood of having\nmeaningful workforce participation outside the region")+
...
ggsave(filename = "Charts/Paper report/mca-m-wf-mn.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report/mca-m-wf-mn.png", dpi = "print", width = 6, height = 4.5)
