...
data <- original %>%
  ...
  ungroup() %>%
  mutate(pct = n / sum(n),
         grad.year.5 = ifelse(grad.year.5 == "Not meaningful, not attending ps", "Not meaningful",
                              ifelse(grad.year.5 == "No MN emp record, not attending ps", "No MN employment",
                                     ifelse(grad.year.5 == "Meaningful WF - Northeast", "Meaningful WF - NE", grad.year.5))),
         grad.year.5 = fct_relevel(grad.year.5, "Meaningful WF - NE", "Meaningful WF - MN", "Not meaningful", "No MN employment"))



# Create chart ------------------------------------------------------------
ggplot(data = data, aes(grad.year.5, pct, fill = grad.year.5)) +
  ...
  labs(x="", 
       y = "", 
       title = "Percent of non-college grads by workforce participation\ncategory",
       subtitle = "Nearly 60% of non-college grads end up without meaningful workforce\nparticipation in Minnesota") +
...
ggsave(filename = "Charts/Paper report/non-college-grads-wf-state.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 4.5)

ggsave(filename = "Charts/Paper report/non-college-grads-wf-state.png", dpi = "print", width = 6.5, height = 4.5)
