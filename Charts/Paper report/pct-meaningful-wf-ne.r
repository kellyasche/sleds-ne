...
ggplot(data = filter(data, states == "Sustained WF - NE"), aes(year.after.hs, pct)) +
  geom_col(position = "dodge", fill = "#d7191c") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), size = 3) +
  labs(x="", 
       y = "",
       title = "Share of graduates with Sustained WF - NE, by years since high school\ngraduation",
       subtitle = "Sustained workforce participation in NE increases in the years after high school.")+
  ylim(c(0, .33)) +
...
ggsave(filename = "Charts/Paper report/pct-meaningful-wf-ne.pdf", device = cairo_pdf, dpi = "print", width = 6.5, height = 4)

ggsave(filename = "Charts/Paper report/pct-meaningful-wf-ne.png", dpi = "print", width = 6.5, height = 4)
