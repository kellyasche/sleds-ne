...
data <- original %>%
  ...
  select(grad.year.5, RaceEthnicity, economic.status, SpecialEdStatus, Gender, Dem_Desc, avg.wages.pct.state, avg.unemp.rate, pseo.participant, cte.achievement, MCA.M) %>%
...
ggplot(data = filter(or_plot, term %in% c("Dem_DescUrban/town/rural mix", "CTE: CTE Concentrator or Completor", "CTE: CTE Participant", "Dem_DescTown/rural mix")), aes(x = estimate, y = term, color = color)) +
  ...
  annotate(geom = "text",
           x = .5,
           y = .6,
           label = "More likely to NOT have sustained\nworkforce participation in region",
           size = 2) +
  annotate(geom = "text",
           x = 1.5,
           y = .6,
           label = "More likely to have sustained\nworkforce participation in the region",
           size = 2) +
  geom_point() +
  scale_color_manual(values = c("#2E7C63")) +
  scale_y_discrete(expand = expansion(add = c(1, .25)),
                   labels = c("Town/rural mix", "CTE Participant","Urban/town/rural mix", "CTE Concentrator")) +
  ...
  labs(
    x = "",
    y = NULL,
    title = "Odds of Attaining Sustained Workforce Participation in the Region\nAmong Non-College Completers",
    subtitle = "CTE engagement and high school rural-ness are associated with a higher liklihood of\nhaving sustained workforce participation in the region 5-years after high school."
  ) +
  theme(legend.position = "none")


ggsave(filename = "Charts/Paper report 3/ps-nongrad-sustained-wf-odds-ratio.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report 3/ps-nongrad-sustained-wf-odds-ratio.png",  dpi = "print", width = 6, height = 4.5)
