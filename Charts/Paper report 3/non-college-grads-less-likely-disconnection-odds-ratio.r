...
ggplot(data = filter(or_plot, term %in% c("attended.psYes", "CTE: CTE Concentrator or Completor", "CTE: CTE Participant")), aes(x = estimate, y = term, color = color)) +
  ...
  annotate(geom = "text",
           x = .25,
           y = .6,
           label = "Less likely to be disconnected",
           size = 2) +
  geom_vline(xintercept = 1, linetype = "dashed") +
  geom_point() +
  scale_color_manual(values = c("#8B601F", "#2E7C63")) +
  scale_y_discrete(labels = c("Attended post secondary\nbut did not complete", "CTE Concentrator", "CTE Participant")) +
  ...
  labs(
    ...
    subtitle = "Attending post-secondary but not completing, and CTE participation are all associated\nwith lower likelihood of being persistently or broadly disconnected."
  ) +
  theme(legend.position = "none")

ggsave(filename = "Charts/Paper report 3/non-college-grads-less-likely-disconnection-odds-ratio.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report 3/non-college-grads-less-likely-disconnection-odds-ratio.png",  dpi = "print", width = 6, height = 4.5)
