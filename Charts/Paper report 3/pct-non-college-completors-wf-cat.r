...
data <- original %>%
  ...
  mutate(pct = n / sum(n),
         ...
         alpha = ifelse(grad.year.5 %in% c("Sustained WF - NE", "Sustained WF - MN"), "Highlight", "No highlight")) 
...
ggplot(data, aes(grad.year.5, pct, fill = grad.year.5, alpha = alpha)) +
  ...
  labs(x="",
       y = "", 
       title = "Percent of Non-College Completors by Workforce Participation\nCategories 5-years After High School", 
       subtitle = "Just over a quarter of non-college completers had sustained workforce participation\nin the region and another 13% outside the region but within Minnesota.")+
...
ggsave(filename = "Charts/Paper report 3/pct-non-college-completors-wf-cat.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Paper report 3/pct-non-college-completors-wf-cat.png", dpi = "print", width = 6, height = 4)
