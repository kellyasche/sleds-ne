...
ggplot(data = filter(data, ps.grad == "Yes"), aes(Gender, pct, fill = Gender)) +
  geom_col(position = "dodge") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), fill = "white", color = "#222222", size = 3) +
  labs(x="", 
       y = "", 
       title = "Percent of high school graduates from 2008 to 2019 that completed\ncollege by gender", 
       subtitle = "A signficantly higher percentage of females complete college than males.")+
...
ggsave(filename = "Charts/Paper report 3/college-grad-gender.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Paper report 3/college-grad-gender.png", dpi = "print", width = 6, height = 4)
