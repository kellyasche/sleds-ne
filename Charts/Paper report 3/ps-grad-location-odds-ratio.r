...
data <- original %>%
  ...
  select(RaceEthnicity, economic.status, SpecialEdStatus, Gender, MCA.M, pseo.participant, cte.achievement, avg.unemp.rate, ps.grad.location) %>%
  mutate(RaceEthnicity = fct_relevel(RaceEthnicity, "White"),
         ...
         MCA.M = ifelse(MCA.M > 2, "Proficient", "Not proficient"),
         MCA.M = fct_relevel(MCA.M, "Not proficient"),
         ...
         ps.grad.location = ifelse(ps.grad.location == "PS grad Region", "College - region", "College - outside region"),
         ps.grad.location = fct_relevel(ps.grad.location, "College - outside region"))
...
ggplot(data = filter(or_plot, term %in% c("CTE: CTE Concentrator or Completor", "Economic status: FRL", "Special Ed: Special Ed", "CTE: CTE Participant", "PSEO: PSEO", "MCA MathProficient")), aes(x = estimate, y = term, color = color)) +
  ...
  annotate(geom = "text",
           ...
           size = 2) +
   geom_point() +
  scale_color_manual(values = c("#8B601F", "#2E7C63")) +
  scale_y_discrete(labels = c("MCA Math - Proficient", "PSEO Participant", "CTE Participant", "Special Ed", "Free-Reduced Lunch", "CTE Concentrator"),
                   expand = expansion(add = c(1, .25))) +
  scale_x_continuous(expand = expansion(add = c(.5, 0)),
                     labels = c("0.0", "0.5", "1.0\nno difference", "1.5", "2.0")) +
   theme_line +
  labs(
    x = "",
    y = NULL,
    title = "Odds of graduating from a regional college compared to graduating\noutside the region, among college completers",
    subtitle = "CTE engagement and economic constraints are associated with graduating college in\nthe region, while academic mobility is associated with leaving"
  ) +
  theme(legend.position = "none")

ggsave(filename = "Charts/Paper report 3/ps-grad-location-odds-ratio.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4.5)

ggsave(filename = "Charts/Paper report 3/ps-grad-location-odds-ratio.png",  dpi = "print", width = 6, height = 4.5)
