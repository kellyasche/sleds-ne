...
ggplot(regions) +
  geom_sf(color = "grey85", fill = "transparent", aes(geometry = geometry)) +
  geom_sf(data = filter(regions, project.pr != "Northeast"), color = "grey85", aes(geometry = geometry, fill = project.pr)) +
  scale_fill_manual(values = c("#009933", "#009933", "#009933", "#009933", "#009933"))+
  theme_sf+
  labs(title= "Not Northeast") +
  theme(legend.position = "none",
        text = element_text(size = 16))

ggsave(filename = "Charts/Presentations/not-northeast-project-region-map.pdf", device = cairo_pdf, dpi = "print", width = 8.5, height = 6.5)
