...
data <- master %>%
  select(cte.achievement, grad.year.10) %>%
  filter(!is.na(cte.achievement)) %>%
  ...
  group_by(grad.year.10, cte.achievement) %>%
  summarize(n = n()) %>%
  ungroup() %>%
...
ggplot(data, aes(grad.year.10, pct, fill = grad.year.10)) +
  facet_wrap(~cte.achievement) +
  geom_bar(stat = "identity") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, color = "white") +
  labs(x="", y = "", title = "Percent of CTE Achievement by employment state\n10 years after graduating high school", subtitle = "CTE engagement important for local meaningful labor force participation", caption = "\nOffice of Higher Education - SLEDS") +
...
ggsave(filename = "Charts/EPIC/cte-achievement-grad-year-10.pdf", device = cairo_pdf, dpi = "print", width = 8.5, height = 7)
