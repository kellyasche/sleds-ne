...
data <- master %>%
  select(ps.grad.location, grad.year.10) %>%
  filter(!is.na(ps.grad.location)) %>%
  filter(ps.grad.location != "Attending ps") %>%
  filter(grad.year.10 != "After 2023") %>%
  filter(grad.year.10 != "Attending ps") %>%
  mutate(ps.grad.location = str_replace(ps.grad.location, "PS grad EDR", "PS grad region")) %>%
  group_by(grad.year.10, ps.grad.location) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  ...
  mutate(grad.year.10 = str_replace(grad.year.10, "No MN emp record, not attending ps", "No MN emp record"),
         ...
         ps.grad.location = fct_relevel(ps.grad.location, "PS grad region", "PS grad MN", "PS grad outside MN", "Did not graduate ps"))


# Create chart ------------------------------------------------------------

ggplot(data, aes(grad.year.10, pct, fill = grad.year.10)) +
  facet_wrap(~ps.grad.location) +
  geom_bar(stat = "identity") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, color = "white") +
  labs(x="", y = "", title = "Percent of graduates by employment state - 10 years after\ngraduating high school", subtitle = "Graduating from a local college increases liklihood of regional employment", caption = "\nOffice of Higher Education - SLEDS") +
...
ggsave(filename = "Charts/EPIC/ps-grad-location-grad-year-10.pdf", device = cairo_pdf, dpi = "print", width = 8.5, height = 7)
