...
ggplot(data = filter(data, emp.states != "Attending ps"), aes(grad.year, pct, fill = emp.states)) +
  facet_wrap(~emp.states, ncol = 2) +
  geom_bar(stat = "identity") +
  geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, color = "white") +
  labs(x="", y = "", title = "Proportion of Central high school students in each employment state",
       caption = "MN Statewide Longitudinal Education Data System (SLEDS)")  +
...
ggsave(filename = "Charts/Presentations/annual-emp-states-bar.pdf", device = cairo_pdf, dpi = "print", width = 12, height = 6.5)
