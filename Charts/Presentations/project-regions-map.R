...
ggplot(regions) +
  geom_sf(color = "grey85", aes(fill = project.pr, geometry = geometry)) +
  scale_fill_manual(values = color.project.pr)+
  theme_sf+
  labs(title= "Defined project regions",
       subtitle = "Project regions are not the same as planning regions") +
  theme(legend.position = "bottom",
        text = element_text(size = 16))

ggsave(filename = "Charts/EPIC/project-regions-map.pdf", device = cairo_pdf, dpi = "print", width = 8.5, height = 6.5)
