...
data <- original %>%
  mutate(states = as.factor(states),
         emp.states = ifelse(states %in% c("Meaningful emp County", "Meaningful emp EDR", "Meaningful emp region"), "Meaningful emp - Region", as.character(states)),
         emp.states = as.factor(emp.states)) %>%
  filter(states != "After 2023") %>%
  group_by(grad.year, emp.states) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(grad.year) %>%
  mutate(pct = n / sum(n)) %>%
  ...
  mutate(emp.states = str_replace(emp.states, "Not meaningful, not attending ps", "Not meaningful WF"),
         emp.states = str_replace(emp.states, "No MN emp record, not attending ps", "No MN employment"),
         emp.states = fct_relevel(emp.states,  "Meaningful emp - Region", "Meaningful emp MN", "Not meaningful WF", "No MN employment", "Attending ps")) %>%
  group_by(grad.year) %>%
  complete(emp.states, fill = list(n = 0, pct = 0, data_id = 0)) %>%
  arrange(desc(emp.states)) %>%
  mutate(cum.pct = cumsum(pct),
         x.loc = cum.pct - ((cum.pct - lag(cum.pct)) / 2),
         x.loc = ifelse(emp.states == "Attending ps", pct, x.loc)) %>%
  ungroup()

ggplot(data, aes(as.numeric(grad.year), pct, fill = emp.states, group = emp.states)) +
  geom_area_interactive(aes(data_id = data_id, tooltip = paste("State: ", emp.states, "\n", percent(pct, accuracy = .1), sep = ""))) +
  geom_label(data = filter(data, grad.year %in% c(1, 5, 10)), aes(x = as.numeric(grad.year), y = x.loc, label = paste(grad.year, "\n", percent(pct, accuracy = .1))), show.legend = FALSE) +
  labs(x="Grad Year", y = "Proportion of individuals", color="", title = "Percentage of individuals in each pathway after graduating high school")+
...
ggsave(filename = "Charts/Presentations/annual-emp-states-prop.pdf", device = cairo_pdf, dpi = "print", width = 12, height = 5.5)
