...
or_plot <- tidy(model3_non_sustained, conf.int = TRUE, exponentiate = TRUE) %>%
...
ggplot(data = filter(or_plot, term %in% c("RaceEthnicityBlack", "SpecialEdStatus1", "RaceEthnicityAI", "GenderF", "RaceEthnicityHispanic", "RaceEthnicityAsian/PI")), aes(x = estimate, y = term, color = color)) +
  ...
  annotate(geom = "text",
           x = .5,
           y = .6,
           label = "Less likely for persistent\nnon-sustained wf participation",
           size = 2) +
  annotate(geom = "text",
           x = 1.5,
           y = .6,
           label = "More likely for persistent\nnon-sustained wf participation",
           size = 2) +
  geom_point() +
  scale_color_manual(values = c("Higher odds" = "#8B601F",
                                "Lower odds" = "#2E7C63",
                                "Not statistically significant" = "grey70")) +
  scale_y_discrete(labels = c("Asian/PI","Hispanic", "Female", "American Indian", "Special Education", "Black"),
  ...
  labs(
    ...
    y = NULL,
    color = "",
    title = "Odds of having persistent non-sustained workforce participation\nfirst five years after formal education",
    subtitle = "These four groups are more likely to cycle through unstable work than their peers."
  ) +
  theme(legend.position = "bottom")

ggsave(filename = "Charts/Draft 4/persistent-non-sustained-wf-odds-ratio.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Draft 4/persistent-non-sustained-wf-odds-ratio.png", dpi = "print", width = 6, height = 4)
