...
grad.emp.location.match <- grad.locations %>%
  ...
  mutate(region.match = new.mn.emp.record,
         new.region.match = ifelse(region.match == "Meaningful employment" & emp.pr %in% c("Northeast"), "Meaningful emp - match", region.match),
...
The chart shows that the percentage of Northeast graduates that have meaningful employment in the same county as their high school grows from 6% the year of graduation to 21% by 15 years after graduating.

The percentage that has meaningful employment outside the county grows from 2% to 26%.
...
The percentage of individuals that have meaningful employment in the region grows from 7% the year of graduation to 27% 15 years after graduating. The percentage with meaningful employent outside the region grows from 2% to 20%.
...
The percentage with meaningful employment in the state grows from 9% to 47%. Over a third don't have a MN employment record.
