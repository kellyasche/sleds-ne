...
- **High school characteristics**
  ...
  - `range.school` : Whether a school is located on the "Iron Range" of Northeast Minnesota.
...
- *`HomelessIndicator`:* 1.2% indicated as homeless between 10th and 12th grade.
- *`LimitedEnglishProficiencyIndicator`: Only .2% were categorized as having limited English.*
- `non.english.home`: 0.8% were categorized as having someone in their household where English wasn't the primary language spoken.
- *`ps.grad.Institute`:* Categories 3,5,6,7,8,9 have less than 1% of graduates, each.
- `RaceEthnicity`: Asian/PI, Black, and Hispanic all had less than 1%.
- `sat.taken`: 1.7% took the ACT
...
- `ps.grad.InstitutionSector`: combine categories 3, 5, 6, 7, 8 and 9 into one category. This means that the following institution sectors will be relabeled as "11";
  ...
  - 8 - Private not-for-profit less than 2-year
  - 9 - Private for-profit, less-than 2-year
- `RaceEthnicity`: I'm going to leave this for now due to potential significant differences in pathways by race and ethnicity.
- `hs.grad.year`: eliminate from analysis

<br>

```{r updated ind qual var}
master.ind.qual.var <- ind.qual.var %>%
  select(-PersonID, -hs.grad.year, -LimitedEnglishProficiencyIndicator, -HomelessIndicator, -non.english.home , -sat.taken, -edr) %>%
  ...
  select(Gender, grad.year.covid, economic.status, SpecialEdStatus, RaceEthnicity, english.learner, Dem_Desc, range.school,  pseo.participant, took.ACT, ap.exam, cte.achievement, attended.ps.within.first.year.hsgrad, attended.ps, ps.grad, ps.grad.InstitutionSector, highest.cred.level, ps.grad.location)
...
Below are the variables that are categorized as "demographics". When I initially did this analysis, the category "Unknown" in the RaceEthnicity variable was throwing off the analysis.

```{r demographics prep, echo=TRUE}
demographics <- master.ind.qual.var %>%
  select(Gender, grad.year.covid, economic.status, SpecialEdStatus, RaceEthnicity, english.learner) 
...
The values below indicate that the first dimension explains nearly 19% of the variation. It takes 6 dimensions to explain 75%. This means the dataset is complex and variable redundancy may be low.
...
Let's take a look to see which variables are going to be important. The chart and table below show the cos2 value for each of the first four dimensions.
...
**Dimension 1:** Pre- or post-COVID grad, White

**Dimension 2:** Special Education and/or english learner and/or poverty

**Dimension 3:** American Indian and poverty

**Dimension 4:** Asian and Pacific Islanders
...
Next, lets see how much each variable is contributing to each dimension. There are 18 categories, so that means a percentage that is above 5.5% indicates a strong contribution to the dimension.

Here are labels that seem to explain each dimension;

1.  Pre- or post-COVID grad
2.  Combination of needing special education, needing additional english language skills, and poor.
3.  Being low-income and American Indian
4.  Being Asian and/or Pacific Islander
...
The values below indicate that the first two dimensions explain 76% of the variance. Examining the cos2 values for each dimension might allow us to eliminate one of these variables.
...
**Dimension 1:** More urban and whether the location is a range school.

**Dimension 2:** More rural districts.
...
Next, lets see how much each variable is contributing to each dimension. There are 6 categories so any percent above 16.67% would be quite influential. It's interesting that EDR 7W and Urban/town/rural mix don't contribute much to either dimension. Here are the labels for each dimension;

1.  More urban and located on the range
2.  More rural and not located on the range
...
1.  Preparing for college or not: ap exam, ACT, PSEO
2.  Relationship between CTE and PSEO
3.  Less engagement with CTE and no PSEO
...
- Graduated from any of the following post-secondary institutions;
  ...
  - graduated from multiple sectors
  - graduated from any of the "11"
  - did not graduate.
- Received a master degree or higher
- Did not attended post-secondary within the first year after high school.

The type of institution an individual graduated from variable is a good candidate for collapsing into fewer categories as well as the highest credential earned variable.
...
Next, lets see how much each variable is contributing to each dimension. There are 23 categories so any percent above 4% would be quite influential. There are a few categories that don't reach that threshold for any of the four dimensions.

- ps.grad.InstitutionSector = 2
- ps.grad.InstitutionSector = 10
- highest.cred.level = Master degree or higher
- ps.grad.InstitutionSector = 11
- ps.grad.InstitutionSector = NA
...
master.final.after.mca <- master.original %>%
  select(-LimitedEnglishProficiencyIndicator, -HomelessIndicator, -non.english.home, -hs.grad.year, sat.taken) %>%
