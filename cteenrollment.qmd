...
career.cluster.codes.names <- read_xlsx('/Users/kellyasche/Library/CloudStorage/GoogleDrive-kasche@ruralmn.org/My Drive/Research/FY25-27/FY25/OHE Journey to meaningful employment/Data/SLEDS/CTEEnrollment/career-cluster-codes-names.xlsx')
...
There is a significant difference in whether a student has taken at least one CTE course and their high school county RUCA category. Although 71% of all graduates took at least one CTE course, that percentage is highest in more rural districts. 86.4% took at least one CTE course in entirely rural districts while 82.0% took at least one in town/rural mix districts. 66.1% took at least one in our urban/town/rural mix districts.

Of those students that took at least 1 CTE course, the average number of CTE courses taken ranged from 3.9 in our entirely rural districts to 3.0 in urban/town/rural mix districts.
...
cte.cf.prop <- master.4 %>%
  gather(key = "CareerCluster", value = "n.courses", cte.1:cte.21) %>% 
...
Not surprisingly, Transportation, Distribution & Logistics was the careercluster with the highest percentage of courses taken with 25.3%. This was followed by manfacturing with 14.1% and architecture and construction with 13.3% of CTE courses taken in that CareerCluster.

<br>

```{r cte CareerClusters concentration, echo=FALSE}
cte.cf.concentration.total <- master.4 %>%
  select(PersonID, cte.1:cte.21) %>%
  gather(key = "CareerCluster", value = "n.courses", cte.1:cte.21) %>%
...
The county maps do show some differences in percentages across career clusters. Manufacturing was consistently offered and taken across the region, as well as transportation, distribution and logistics.
...
cte.cf.concentration.county <- master.4 %>%
  select(PersonID, county.name, countyfp, cte.1:cte.21) %>%
  gather(key = "CareerCluster", value = "n.courses", cte.1:cte.21) %>%
...
cte.cf.concentration.year <- master.4 %>%
  select(PersonID, grad.year, cte.1:cte.21) %>%
  gather(key = "CareerCluster", value = "n.courses", cte.1:cte.21) %>%
