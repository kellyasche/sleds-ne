...
k12organization <- read_csv("~/Library/CloudStorage/GoogleDrive-kasche@ruralmn.org/My Drive/Research/FY25-27/FY27/OHE Journey to meaningful employment/Data/SLEDS/K12 Orgs/K12Organization.csv") 
...
master <- master.original %>%
  ...
  select(PersonID:planning.region, range.school, hs.grad.year:wages.3year.avg)

range.schools <- master %>%
  select(PersonID, range.school)

write_csv(range.schools, "Data/SLEDS/Masters/range-schools_master.csv")
```
