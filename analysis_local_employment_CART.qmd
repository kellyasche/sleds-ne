...
# Meaningful emp in MN vs. meaningful emp in NE
...
meaningful.emp.mn.grad.year.1.prop
...
## Important variables through time

The following table provides the level of important for each grad.year.x in the above analysis. Here are the primary themes;

-   Variables that are important one year after graduation are less so in the following years. A few to just avoid because they don't do much after one year;
    -   grad.year.covid
    -   ps.grad (interesting)
-   There are two variables that are important five and ten years after graduating high school;
    -   ps.grad.location
    -   highest.cred.earned
-   A couple of variables that stay either important or less so throughout the years
    -   MCA.M
    -   Dem_Desc
    -   RaceEthnicity
    -   cte.achievement
    -   ACTCompositeScore
    -   total.cte.courses.taken
-   Variables that had some importance five and ten years after graduating high school
    -   SpecialEdStatus
    -   took.ACT
    -   MCA.R

The variables to keep by group are;
...
```{r create table for importance variables over time}

ind.var.table <- names(ind.var) %>%
  as_tibble() %>%
  rename(ind.var = 1) %>%
  mutate(ind.var = as_factor(ind.var),
         var.group = ifelse(ind.var %in% c("grad.year.covid", "economic.status", "SpecialEdStatus", "RaceEthnicity", "english.learner"), "Demographics",
                            ifelse(ind.var %in% c("range.school", "Dem_Desc"), "HS Characteristics",
                                   ifelse(ind.var %in% c("pseo.participant", "took.ACT", "ACTCompositeScore", "MCA.M", "MCA.R", "MCA.S"), "Prep for PS",
                                          ifelse(ind.var %in% c("cte.achievement", "total.cte.courses.taken"), "CTE Engagement", "Post-secondary path")))),
         year.one = ifelse(ind.var %in% c("ACTCompositeScore", "Dem_Desc", "total.cte.courses.taken", "MCA.S", "ps.grad"), "Important",
                           ifelse(ind.var %in% c("RaceEthnicity", "MCA.M", "cte.achievement", "economic status", "grad.year.covid"), "Less so", "")),
         year.five = ifelse(ind.var %in% c("ps.grad.location", "ps.grad.InstitutionSector", "highest.cred.level", "MCA.M", "Dem_Desc"), "Important",
                            ifelse(ind.var %in% c("MCA.R", "ACTCompositeScore", "cte.achievement", "took.ACT", "total.cte.courses.taken", "SpecialEdStatus", "english.learner", "MCA.S", "RaceEthnicity"), "Less so", "")),
         year.ten = ifelse(ind.var %in% c("ps.grad.location", "ps.grad.InstituitionSector", "highest.cred.level"), "Important",
                           ifelse(ind.var %in% c("MCA.M", "ACTCompositeScore", "MCA.R", "Dem_Desc", "economic.status", "took.ACT", "ps.grad", "SpecialEdStatus", "cte.achievement", "total.cte.courses.taken", "RaceEthnicity"), "Less so", "")))

datatable(ind.var.table, class = "cell-border stripe", filter = "top", rownames = FALSE,
          options = list(
...
# Not meaningful or attending post-secondary vs meaningful emp in NE
...
It probably doesn't make sense to have a dataset with a bunch of post-secondary variables in it when exploring which variables make a difference in whether someone has meaningful workforce participation in the region one year after graduating high school. So, I'm going to delete all of them.

```{r prep not meaningful emp grad year 1}
not.meaningful.emp.grad.year.1 <- master %>%
  filter(grad.year.1 %in% c("Meaningful emp NE", "Not meaningful, not attending ps")) %>%
  select(-PersonID, -grad.year.5, -grad.year.10, -ps.grad.InstitutionSector, -ps.grad.location, -highest.cred.level, -ps.grad) %>%
  droplevels()

kable(names(not.meaningful.emp.grad.year.1))
```

When filtering for individuals that had meaningful employment in NE vs MN one year after graduation we end up with `r comma(nrow(not.meaningful.emp.grad.year.1))` individuals.The table below shows that a a minority of individuals have meaningful employment in the region - 42% compared to individuals with no meaningful employment and are not attending post-secondary.

<br>

```{r not meaningful emp grad year 1 prop}
not.meaningful.emp.grad.year.1.prop <- not.meaningful.emp.grad.year.1 %>%
  group_by(grad.year.1) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  mutate(pct = n / sum(n),
...
not.meaningful.emp.grad.year.1.prop
```
...
```{r split dataset into test and train not meaningful emp grad year 1, echo=TRUE}
set.seed(1234)

sample_ind <- sample(nrow(not.meaningful.emp.grad.year.1), nrow(not.meaningful.emp.grad.year.1) *.7)

train <- not.meaningful.emp.grad.year.1[sample_ind,]

test <- not.meaningful.emp.grad.year.1[-sample_ind,]
...
The summary shows that the lowest cross-validated error is .96908 and the nsplit is 3.

<br>

```{r create decision tree not meaningful emp grad year 1, echo=TRUE, include = FALSE}
#Base model

not.meaningful.emp.grad.year.1.model <- rpart(grad.year.1 ~ ., data = train, method = "class", control = rpart.control(cp = 0))

#Summary
summary(not.meaningful.emp.grad.year.1.model)

```

```{r decision tree plot not meaningful emp grad year 1}
#Plot tree
rpart.plot(not.meaningful.emp.grad.year.1.model)

# Examine the complexity plot
printcp(not.meaningful.emp.grad.year.1.model)

plotcp(not.meaningful.emp.grad.year.1.model)
...
```{r base model variable importance not meaningful emp grad year 1, echo=TRUE}

not.meaningful.emp.grad.year.1.model$variable.importance

importance <- not.meaningful.emp.grad.year.1.model$variable.importance
...
```{r model accuracy not meaningful emp grad year 1, echo=TRUE}
test$pred <- predict(not.meaningful.emp.grad.year.1.model, test, type = "class")
...
-   maxdepth: 3 to 6
-   minsplit: 5% of n which is 503
-   minbucket: round(minsplit / 3) which is 168
...
```{r pre pruning not meaningful emp grad year 1, echo=TRUE}
not.meaningful.emp.grad.year.1.model.preprune <- rpart(grad.year.1 ~ ., data = train, method = "class", 
                   control = rpart.control(cp = 0, maxdepth = 6, minsplit = 503, minbucket = 168))

test$pred <- predict(not.meaningful.emp.grad.year.1.model.preprune, test, type = "class")
...
```{r pre pruning summary not meaningful emp grad year 1, include=FALSE}

#Summary
summary(not.meaningful.emp.grad.year.1.model.preprune)

```

The pre-pruning resulted in a significantly less complex tree with a .6014589 accuracy. Let's see what variables are important in this model.

```{r pre pruning plot not meaningful emp  grad year 1, echo=TRUE}
#Plot tree
rpart.plot(not.meaningful.emp.grad.year.1.model.preprune)

# Examine the complexity plot
printcp(not.meaningful.emp.grad.year.1.model.preprune)
```

The nodes show;

-   SpecialEdStatus = 0
    -   No: significantly more likely to not have meaningful employment or in post-secondary
    -   Yes:
        -   Meaningful emp in NE: range.school, economic.status = 0, total.cte.courses.taken \> 4

        -   Not meaningful: non-white, took less cte courses, not a range school
...
```{r pre pruning variable importance not meaningful emp grad year 1, echo=TRUE}
not.meaningful.emp.grad.year.1.model.preprune$variable.importance

importance <- not.meaningful.emp.grad.year.1.model.preprune$variable.importance
...
```{r postpruning not meaningful emp grad year 1, echo=TRUE}

cp_table <- not.meaningful.emp.grad.year.1.model$cptable
...
optimal_cp <- not.meaningful.emp.grad.year.1.model$cptable[which.min(not.meaningful.emp.grad.year.1.model$cptable[, "xerror"]), "CP"]


# Prune the hr_base_model based on the optimal cp value
not.meaningful.emp.grad.year.1.model.postprune <- prune(not.meaningful.emp.grad.year.1.model, cp = optimal_cp)

#Summary
summary(not.meaningful.emp.grad.year.1.model.postprune)

#Plot tree
rpart.plot(not.meaningful.emp.grad.year.1.model.postprune, nn = TRUE)

# Examine the complexity plot
printcp(not.meaningful.emp.grad.year.1.model.postprune)
plotcp(not.meaningful.emp.grad.year.1.model.postprune)


# Compute the accuracy of the pruned tree

test$pred <- predict(not.meaningful.emp.grad.year.1.model.postprune, test, type = "class")
...
-   SpecialEdStatus = 0
    -   No: not meaningful employment
    -   Yes:
        -   Meaningful emp: is a range.school \| lower ACTComposite Score
        -   Not meaningful emp: not a range school \| higher ACTCompositeScore
...
```{r post pruning variable importance not meaningful emp grad year 1, echo=TRUE}
not.meaningful.emp.grad.year.1.model.postprune$variable.importance

importance <- not.meaningful.emp.grad.year.1.model.postprune$variable.importance
...
```{r compare accuracy not meaningful emp  grad year 1, echo=TRUE}
data.frame(base_accuracy, accuracy_preprun, accuracy_postprun)
...
## Five year after graduation

We are now five years away from graduating high school, so we can add the post-secondary variables back in.

```{r prep not meaningful emp grad year 5}
not.meaningful.emp.grad.year.5 <- master %>%
  filter(grad.year.5 %in% c("Meaningful emp NE", "Not meaningful, not attending ps")) %>%
  select(-PersonID, -grad.year.1, -grad.year.10) %>%
  droplevels()

kable(names(not.meaningful.emp.grad.year.5))
```

When filtering for individuals that had meaningful employment in NE vs MN one year after graduation we end up with `r comma(nrow(not.meaningful.emp.grad.year.5))` individuals.The table below shows that a majority of individuals have meaningful employment in the region - 55% compared to individuals with no meaningful employment and are not attending post-secondary.

<br>

```{r not meaningful emp grad year 5 prop}
not.meaningful.emp.grad.year.5.prop <- not.meaningful.emp.grad.year.5 %>%
  group_by(grad.year.5) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  mutate(pct = n / sum(n),
...
not.meaningful.emp.grad.year.5.prop
```
...
```{r split dataset into test and train not meaningful emp grad year 5, echo=TRUE}
set.seed(1234)

sample_ind <- sample(nrow(not.meaningful.emp.grad.year.5), nrow(not.meaningful.emp.grad.year.5) *.7)

train <- not.meaningful.emp.grad.year.5[sample_ind,]

test <- not.meaningful.emp.grad.year.5[-sample_ind,]
...
The summary shows that the lowest cross-validated error is .0.86710 and the nsplit is 25.

<br>

```{r create decision tree not meaningful emp grad year 5, echo=TRUE, include = FALSE}
#Base model

not.meaningful.emp.grad.year.5.model <- rpart(grad.year.5 ~ ., data = train, method = "class", control = rpart.control(cp = 0))

#Summary
summary(not.meaningful.emp.grad.year.5.model)

```

```{r decision tree plot not meaningful emp grad year 5}
#Plot tree
rpart.plot(not.meaningful.emp.grad.year.5.model)

# Examine the complexity plot
printcp(not.meaningful.emp.grad.year.5.model)

plotcp(not.meaningful.emp.grad.year.5.model)
...
```{r base model variable importance not meaningful emp grad year 5, echo=TRUE}

not.meaningful.emp.grad.year.5.model$variable.importance

importance <- not.meaningful.emp.grad.year.5.model$variable.importance
...
```{r model accuracy not meaningful emp grad year 5, echo=TRUE}
test$pred <- predict(not.meaningful.emp.grad.year.5.model, test, type = "class")
...
-   maxdepth: 3 to 6
-   minsplit: 5% of n which is 498
-   minbucket: round(minsplit / 3) which is 166
...
```{r pre pruning not meaningful emp grad year 5, echo=TRUE}
not.meaningful.emp.grad.year.5.model.preprune <- rpart(grad.year.5 ~ ., data = train, method = "class", 
                   control = rpart.control(cp = 0, maxdepth = 6, minsplit = 498, minbucket = 166))

test$pred <- predict(not.meaningful.emp.grad.year.5.model.preprune, test, type = "class")
...
```{r pre pruning summary not meaningful emp grad year 5, include=FALSE}

#Summary
summary(not.meaningful.emp.grad.year.5.model.preprune)

```

The pre-pruning resulted in a significantly less complex tree with a .6014589 accuracy. Let's see what variables are important in this model.

```{r pre pruning plot not meaningful emp  grad year 5, echo=TRUE}
#Plot tree
rpart.plot(not.meaningful.emp.grad.year.5.model.preprune)

# Examine the complexity plot
printcp(not.meaningful.emp.grad.year.5.model.preprune)
```

The nodes show;

-   ps.grad.location = In region \| Inside and outside region
    -   Yes: Meaningful emp in NE
    -   No:
        -   Meaningful emp NE
            -   total.cte.courses \>= 2
                -   economic.status = 0
                    -   ps.grad.InstitutionSector = 11,4,Never attended ps
                -   economic.status = 1
                    -   SpecialEdStatus = 0
                    -   Dem_Desc = Urban/town/rural mix
            -   total.cte.courses \< 2
                -   ACTCompositeScore \< 22
                    -   RaceEthnicity = White
                        -   took.ACT = Yes
                            -   MCA.M \>= 3
...
```{r pre pruning variable importance not meaningful emp grad year 5, echo=TRUE}
not.meaningful.emp.grad.year.5.model.preprune$variable.importance

importance <- not.meaningful.emp.grad.year.5.model.preprune$variable.importance
...
```{r postpruning not meaningful emp grad year 5, echo=TRUE}

cp_table <- not.meaningful.emp.grad.year.5.model$cptable
...
optimal_cp <- not.meaningful.emp.grad.year.5.model$cptable[which.min(not.meaningful.emp.grad.year.5.model$cptable[, "xerror"]), "CP"]


# Prune the hr_base_model based on the optimal cp value
not.meaningful.emp.grad.year.5.model.postprune <- prune(not.meaningful.emp.grad.year.5.model, cp = optimal_cp)

#Summary
summary(not.meaningful.emp.grad.year.5.model.postprune)

#Plot tree
rpart.plot(not.meaningful.emp.grad.year.5.model.postprune, nn = TRUE)

# Examine the complexity plot
printcp(not.meaningful.emp.grad.year.5.model.postprune)
plotcp(not.meaningful.emp.grad.year.5.model.postprune)


# Compute the accuracy of the pruned tree

test$pred <- predict(not.meaningful.emp.grad.year.5.model.postprune, test, type = "class")
...
```{r post pruning variable importance not meaningful emp grad year 5, echo=TRUE}
not.meaningful.emp.grad.year.5.model.postprune$variable.importance

importance <- not.meaningful.emp.grad.year.5.model.postprune$variable.importance
...
```{r compare accuracy not meaningful emp  grad year 5, echo=TRUE}
data.frame(base_accuracy, accuracy_preprun, accuracy_postprun)
...
The pre-pruned tree provided better accuracy.
...
## Ten year after graduation

We are now five years away from graduating high school, so we can add the post-secondary variables back in.

```{r prep not meaningful emp grad year 10}
not.meaningful.emp.grad.year.10 <- master %>%
  filter(grad.year.10 %in% c("Meaningful emp NE", "Not meaningful, not attending ps")) %>%
  select(-PersonID, -grad.year.1, -grad.year.5) %>%
  droplevels()

kable(names(not.meaningful.emp.grad.year.10))
```

When filtering for individuals that had meaningful employment in NE vs MN one year after graduation we end up with `r comma(nrow(not.meaningful.emp.grad.year.10))` individuals.The table below shows that a majority of individuals have meaningful employment in the region - 60% compared to individuals with no meaningful employment and are not attending post-secondary.

<br>

```{r not meaningful emp grad year 10 prop}
not.meaningful.emp.grad.year.10.prop <- not.meaningful.emp.grad.year.10 %>%
  group_by(grad.year.10) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  mutate(pct = n / sum(n),
...
not.meaningful.emp.grad.year.10.prop
```
...
```{r split dataset into test and train not meaningful emp grad year 10, echo=TRUE}
set.seed(1234)

sample_ind <- sample(nrow(not.meaningful.emp.grad.year.10), nrow(not.meaningful.emp.grad.year.10) *.7)

train <- not.meaningful.emp.grad.year.10[sample_ind,]

test <- not.meaningful.emp.grad.year.10[-sample_ind,]
...
The summary shows that the lowest cross-validated error is .0.94194 and the nsplit is 14.

<br>

```{r create decision tree not meaningful emp grad year 10, echo=TRUE, include = FALSE}
#Base model

not.meaningful.emp.grad.year.10.model <- rpart(grad.year.10 ~ ., data = train, method = "class", control = rpart.control(cp = 0))

#Summary
summary(not.meaningful.emp.grad.year.10.model)

```

```{r decision tree plot not meaningful emp grad year 10}
#Plot tree
rpart.plot(not.meaningful.emp.grad.year.10.model)

# Examine the complexity plot
printcp(not.meaningful.emp.grad.year.10.model)

plotcp(not.meaningful.emp.grad.year.10.model)
...
```{r base model variable importance not meaningful emp grad year 10, echo=TRUE}

not.meaningful.emp.grad.year.10.model$variable.importance

importance <- not.meaningful.emp.grad.year.10.model$variable.importance
...
```{r model accuracy not meaningful emp grad year 10, echo=TRUE}
test$pred <- predict(not.meaningful.emp.grad.year.10.model, test, type = "class")
...
-   minsplit: 5% of n which is 281

-   minbucket: round(minsplit / 3) which is 94

<br>

```{r pre pruning not meaningful emp grad year 10, echo=TRUE}
not.meaningful.emp.grad.year.10.model.preprune <- rpart(grad.year.10 ~ ., data = train, method = "class", 
                   control = rpart.control(cp = 0, maxdepth = 6, minsplit = 281, minbucket = 94))

test$pred <- predict(not.meaningful.emp.grad.year.10.model.preprune, test, type = "class")
...
```{r pre pruning summary not meaningful emp grad year 10, include=FALSE}

#Summary
summary(not.meaningful.emp.grad.year.10.model.preprune)

```

The pre-pruning resulted in a significantly less complex tree with a .6014589 accuracy. Let's see what variables are important in this model.

```{r pre pruning plot not meaningful emp  grad year 10, echo=TRUE}
#Plot tree
rpart.plot(not.meaningful.emp.grad.year.10.model.preprune)

# Examine the complexity plot
printcp(not.meaningful.emp.grad.year.10.model.preprune)
```
...
```{r pre pruning variable importance not meaningful emp grad year 10, echo=TRUE}
not.meaningful.emp.grad.year.10.model.preprune$variable.importance

importance <- not.meaningful.emp.grad.year.10.model.preprune$variable.importance
...
```{r postpruning not meaningful emp grad year 10, echo=TRUE}

cp_table <- not.meaningful.emp.grad.year.10.model$cptable
...
optimal_cp <- not.meaningful.emp.grad.year.10.model$cptable[which.min(not.meaningful.emp.grad.year.10.model$cptable[, "xerror"]), "CP"]


# Prune the hr_base_model based on the optimal cp value
not.meaningful.emp.grad.year.10.model.postprune <- prune(not.meaningful.emp.grad.year.10.model, cp = optimal_cp)

#Summary
summary(not.meaningful.emp.grad.year.10.model.postprune)

#Plot tree
rpart.plot(not.meaningful.emp.grad.year.10.model.postprune, nn = TRUE)

# Examine the complexity plot
printcp(not.meaningful.emp.grad.year.10.model.postprune)
plotcp(not.meaningful.emp.grad.year.10.model.postprune)


# Compute the accuracy of the pruned tree

test$pred <- predict(not.meaningful.emp.grad.year.10.model.postprune, test, type = "class")
...
-   ps.grad.location = In region \| Inside and outside region
    ...
    -   No:
        -   Meaningful emp NE
            -   SpecialEdStatus = 0
                -   RaceEthnicity = Asian/PI \| White
                    -   total.cte.courses \>= 3
                    -   total.cte.courses \<= 3
                        -   ACTCompositeScore \< 30
                            -   Economic.status = 0
                            -   Economic.status = 1
                                -   MCA.S \< 3
...
```{r post pruning variable importance not meaningful emp grad year 10, echo=TRUE}
not.meaningful.emp.grad.year.10.model.postprune$variable.importance

importance <- not.meaningful.emp.grad.year.10.model.postprune$variable.importance
...
```{r compare accuracy not meaningful emp  grad year 10, echo=TRUE}
data.frame(base_accuracy, accuracy_preprun, accuracy_postprun)
...
-   Variables that are never important
    -   grad.year.covid
    -   pseo.participante
-   A few that just don't make the cut either
    -   MCA.S
-   One variable that stays important throughout the entire analysis
    -   SpecialEdStatus
-   One variable that starts as important but loses importance over time
    -   ACTCompositeScore
-   There are a few variables that are important five and ten years after graduating high school;
    -   ps.grad.location
    -   highest.cred.earned
    -   ps.grad
    -   ps.grad.InstitutionSector
    -   total.cte.courses.taken
-   A few that are important once and less so once
    -   economic.status
    -   cte.achievement
    -   RaceEthnicity
    -   took.ACT
...
```{r create table for importance variables over time not meaningful emp}
  ...
  mutate(ind.var = as_factor(ind.var),
         var.group = ifelse(ind.var %in% c("grad.year.covid", "economic.status", "SpecialEdStatus", "RaceEthnicity", "english.learner"), "Demographics",
                            ifelse(ind.var %in% c("range.school", "Dem_Desc"), "HS Characteristics",
                                   ifelse(ind.var %in% c("pseo.participant", "took.ACT", "ACTCompositeScore", "MCA.M", "MCA.R", "MCA.S"), "Prep for PS",
                                          ifelse(ind.var %in% c("cte.achievement", "total.cte.courses.taken"), "CTE Engagement", "Post-secondary path")))),
         year.one = ifelse(ind.var %in% c("SpecialEdStatus", "ACTCompositeScore", "range.school", "english.learner"), "Important",
                           ifelse(ind.var %in% c("MCA.R"), "Less so", "")),
         year.five = ifelse(ind.var %in% c("ps.grad.location", "ps.grad.InstitutionSector", "highest.cred.level", "ps.grad", "total.cte.courses.taken", "economic.status", "cte.achievement", "SpecialEdStatus"), "Important",
                            ifelse(ind.var %in% c("took.ACT", "RaceEthnicity", "ACTCompositeScore", "range.school", "MCA.M", "MCA.R", "Dem_Desc", "english.learner", "MCA.S"), "Less so", "")),
         year.ten = ifelse(ind.var %in% c("ps.grad.location", "ps.grad.InstitutionSector", "highest.cred.level", "ps.grad", "SpecialEdStatus", "took.ACT", "total.cte.courses.taken", "RaceEthnicity"), "Important",
                           ifelse(ind.var %in% c("range.school", "cte.achievement", "economic.status", "english.learner", "MCA.M", "MCA.R", "Dem_Desc"), "Less so", "")))
...
# No MN employment record vs meaningful emp in NE
...
```{r prep no MN emp grad year 1}
no.mn.emp.grad.year.1 <- master %>%
  filter(grad.year.1 %in% c("Meaningful emp NE", "No MN emp record, not attending ps")) %>%
  select(-PersonID, -grad.year.5, -grad.year.10, -ps.grad.InstitutionSector, -ps.grad.location, -highest.cred.level, -ps.grad) %>%
  droplevels()

kable(names(no.mn.emp.grad.year.1))
```

When filtering for individuals that had meaningful employment in NE vs MN one year after graduation we end up with `r comma(nrow(no.mn.emp.grad.year.1))` individuals.The table below shows that a a minority of individuals have meaningful employment in the region - 27% compared to individuals with no meaningful employment and are not attending post-secondary.

<br>

```{r no mn emp grad year 1 prop}
no.mn.emp.grad.year.1.prop <- no.mn.emp.grad.year.1 %>%
  group_by(grad.year.1) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  mutate(pct = n / sum(n),
...
```{r split dataset into test and train no mn emp grad year 1, echo=TRUE}
set.seed(1234)
...
The summary shows that the lowest cross-validated error is 1.0072 and the nsplit is 7.

<br>

```{r create decision tree no mn emp grad year 1, echo=TRUE, include = FALSE}
#Base model

no.mn.emp.grad.year.1.model <- rpart(grad.year.1 ~ ., data = train, method = "class", control = rpart.control(cp = 0))

#Summary
summary(no.mn.emp.grad.year.1.model)

```

```{r decision tree plot no mn emp grad year 1}
#Plot tree
rpart.plot(no.mn.emp.grad.year.1.model)
...
```{r base model variable importance no mn emp grad year 1, echo=TRUE}

no.mn.emp.grad.year.1.model$variable.importance

importance <- no.mn.emp.grad.year.1.model$variable.importance
...
```{r model accuracy no mn emp grad year 1, echo=TRUE}
...
-   maxdepth: 3 to 6
-   minsplit: 5% of n which is 787
-   minbucket: round(minsplit / 3) which is 262

Unforttunately, these parameters give me a tree with no splits. So, we are going to use the following;
...
```{r pre pruning no mn emp grad year 1, echo=TRUE}
...
```{r pre pruning summary no mn emp grad year 1, include=FALSE}

#Summary
summary(no.mn.emp.grad.year.1.model.preprune)

```

The pre-pruning resulted in a significantly less complex tree with a .7363405 accuracy. Let's see what variables are important in this model.

```{r pre pruning plot no MN emp  grad year 1, echo=TRUE}
#Plot tree
rpart.plot(no.mn.emp.grad.year.1.model.preprune)
...
```{r pre pruning variable importance no mn emp grad year 1, echo=TRUE}
no.mn.emp.grad.year.1.model.preprune$variable.importance

importance <- no.mn.emp.grad.year.1.model.preprune$variable.importance
...
```{r postpruning no mn emp grad year 1, echo=TRUE}

cp_table <- no.mn.emp.grad.year.1.model$cptable
...
optimal_cp <- no.mn.emp.grad.year.1.model$cptable[which.min(no.mn.emp.grad.year.1.model$cptable[, "xerror"]), "CP"]


# Prune the hr_base_model based on the optimal cp value
no.mn.emp.grad.year.1.model.postprune <- prune(no.mn.emp.grad.year.1.model, cp = .00092764)

#Summary
summary(no.mn.emp.grad.year.1.model.postprune)
...
This is a very complex tree. Honestly, it's too complicated to interpret. But here's a quick summary
...
```{r post pruning variable importance no mn emp grad year 1, echo=TRUE}
no.mn.emp.grad.year.1.model.postprune$variable.importance

importance <- no.mn.emp.grad.year.1.model.postprune$variable.importance
...
```{r compare accuracy no MN emp  grad year 1, echo=TRUE}
data.frame(base_accuracy, accuracy_preprun, accuracy_postprun)
...
The pre-pruned treesprovided better accuracy than the base model so let's use the most important variables from both.
...
```{r prep no mn emp grad year 5}
no.mn.emp.grad.year.5 <- master %>%
  filter(grad.year.5 %in% c("Meaningful emp NE", "No MN emp record, not attending ps")) %>%
  select(-PersonID, -grad.year.1, -grad.year.10) %>%
  droplevels()

kable(names(no.mn.emp.grad.year.5))
```

When filtering for individuals that had meaningful employment in NE vs MN one year after graduation we end up with `r comma(nrow(no.mn.emp.grad.year.5))` individuals.The table below shows that a a minority of individuals have meaningful employment in the region - 27% compared to individuals with no meaningful employment and are not attending post-secondary.

<br>

```{r no mn emp grad year 5 prop}
no.mn.emp.grad.year.5.prop <- no.mn.emp.grad.year.5 %>%
  group_by(grad.year.5) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  mutate(pct = n / sum(n),
...
```{r split dataset into test and train no mn emp grad year 5, echo=TRUE}
set.seed(1234)
...
The summary shows that the lowest cross-validated error is 0.95505 and the nsplit is 11.

<br>

```{r create decision tree no mn emp grad year 5, echo=TRUE, include = FALSE}
#Base model

no.mn.emp.grad.year.5.model <- rpart(grad.year.5 ~ ., data = train, method = "class", control = rpart.control(cp = 0))

#Summary
summary(no.mn.emp.grad.year.5.model)

```

```{r decision tree plot no mn emp grad year 5}
#Plot tree
rpart.plot(no.mn.emp.grad.year.5.model)
...
```{r base model variable importance no mn emp grad year 5, echo=TRUE}

no.mn.emp.grad.year.5.model$variable.importance

importance <- no.mn.emp.grad.year.5.model$variable.importance
...
```{r model accuracy no mn emp grad year 5, echo=TRUE}
...
-   maxdepth: 3 to 6
-   minsplit: 5% of n which is 1030
-   minbucket: round(minsplit / 3) which is 343

<br>

```{r pre pruning no mn emp grad year 5, echo=TRUE}
no.mn.emp.grad.year.5.model.preprune <- rpart(grad.year.5 ~ ., data = train, method = "class", 
                   control = rpart.control(cp = 0, maxdepth = 6, minsplit = 1030, minbucket = 343))
...
```{r pre pruning summary no mn emp grad year 5, include=FALSE}

#Summary
summary(no.mn.emp.grad.year.5.model.preprune)

```

The pre-pruning resulted in a significantly less complex tree with a .7392782 accuracy. Let's see what variables are important in this model.

```{r pre pruning plot no mn emp grad year 5, echo=TRUE}
#Plot tree
rpart.plot(no.mn.emp.grad.year.5.model.preprune)
...
-   ps.grad.location = In region \| Inside and outside region
    -   No: No MN emp record
    -   Yes
        -   range.school = non-range school
            -   Dem_Desc = Urban/town/rural mix
                -   cte.achievement = CTE Concentrator or Completor
                    -   pseo.participant = 0
...
```{r pre pruning variable importance no mn emp grad year 5, echo=TRUE}
no.mn.emp.grad.year.5.model.preprune$variable.importance

importance <- no.mn.emp.grad.year.5.model.preprune$variable.importance
...
```{r postpruning no mn emp grad year 5, echo=TRUE}

cp_table <- no.mn.emp.grad.year.5.model$cptable
...
optimal_cp <- no.mn.emp.grad.year.5.model$cptable[which.min(no.mn.emp.grad.year.5.model$cptable[, "xerror"]), "CP"]


# Prune the hr_base_model based on the optimal cp value
no.mn.emp.grad.year.5.model.postprune <- prune(no.mn.emp.grad.year.5.model, cp = optimal_cp)

#Summary
summary(no.mn.emp.grad.year.5.model.postprune)
...
This is a bit more simpler of a tree compared to the pre-pruned model.
...
-   ps.grad.location = In region \| Inside and outside region
    ...
    -   Yes
        -   range.school = non-range school
            -   Dem_Desc = Urban/Town/Rural mix
                -   cte.achievement: completor or concentrator
                -   cte.achievement: not completetor or concentrator
                    -   pseo.participant = 0
                    -   total.cte.courses.taken \< 6
                    -   RaceEthnicity = AI, Black, White
...
```{r post pruning variable importance no mn emp grad year 5, echo=TRUE}
no.mn.emp.grad.year.5.model.postprune$variable.importance

importance <- no.mn.emp.grad.year.5.model.postprune$variable.importance
...
```{r compare accuracy no MN emp grad year 5, echo=TRUE}
data.frame(base_accuracy, accuracy_preprun, accuracy_postprun)
...
The post-pruned tree provided better accuracy than the base model so let's use the most important variables from both.
...
```{r prep no.mn.emp.grad.year.10}
no.mn.emp.grad.year.10 <- master %>%
  filter(grad.year.10 %in% c("Meaningful emp NE", "No MN emp record, not attending ps")) %>%
  select(-PersonID, -grad.year.1, -grad.year.5) %>%
  droplevels()

kable(names(no.mn.emp.grad.year.10))
```

When filtering for individuals that had meaningful employment in NE vs MN one year after graduation we end up with `r comma(nrow(no.mn.emp.grad.year.5))` individuals.The table below shows that a a minority of individuals have meaningful employment in the region - 22% compared to individuals with no meaningful employment and are not attending post-secondary.

<br>

```{r no.mn.emp.grad.year.10 prop}
no.mn.emp.grad.year.10.prop <- no.mn.emp.grad.year.10 %>%
  group_by(grad.year.10) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  mutate(pct = n / sum(n),
...
```{r split dataset into test and train no.mn.emp.grad.year.10, echo=TRUE}
set.seed(1234)
...
```{r create decision tree no.mn.emp.grad.year.10, echo=TRUE, include = FALSE}
#Base model
...
#Summary
summary(no.mn.emp.grad.year.10.model)

```

```{r decision tree plot no.mn.emp.grad.year.10}
#Plot tree
rpart.plot(no.mn.emp.grad.year.10.model)
...
```{r base model variable importance no.mn.emp.grad.year.10, echo=TRUE}

no.mn.emp.grad.year.10.model$variable.importance

importance <- no.mn.emp.grad.year.10.model$variable.importance
...
```{r model accuracy no.mn.emp.grad.year.10, echo=TRUE}
...
-   maxdepth: 3 to 6
-   minsplit: 5% of n which is 765
-   minbucket: round(minsplit / 3) which is 255

<br>

```{r pre pruning no.mn.emp.grad.year.10, echo=TRUE}
no.mn.emp.grad.year.10.model.preprune <- rpart(grad.year.10 ~ ., data = train, method = "class", 
                   control = rpart.control(cp = 0, maxdepth = 6, minsplit = 765, minbucket = 255))
...
```{r pre pruning summary no.mn.emp.grad.year.10, include=FALSE}

#Summary
summary(no.mn.emp.grad.year.10.model.preprune)

```

The pre-pruning resulted in a significantly less complex tree with a .7782618 accuracy. Let's see what variables are important in this model.

```{r pre pruning plot no MN emp  grad year 10, echo=TRUE}
#Plot tree
rpart.plot(no.mn.emp.grad.year.10.model.preprune)
...
```{r pre pruning variable importance no.mn.emp.grad.year.10, echo=TRUE}
no.mn.emp.grad.year.10.model.preprune$variable.importance

importance <- no.mn.emp.grad.year.10.model.preprune$variable.importance
...
```{r postpruning no.mn.emp.grad.year.10, echo=TRUE}

cp_table <- no.mn.emp.grad.year.10.model$cptable
...
optimal_cp <- no.mn.emp.grad.year.10.model$cptable[which.min(no.mn.emp.grad.year.10.model$cptable[, "xerror"]), "CP"]


# Prune the hr_base_model based on the optimal cp value
no.mn.emp.grad.year.10.model.postprune <- prune(no.mn.emp.grad.year.10.model, cp = optimal_cp)

#Summary
summary(no.mn.emp.grad.year.10.model.postprune)
...
This is a bit more complex of a tree compared to the pre-pruned model. Honestly, it's too complex to interpret.
...
```{r post pruning variable importance no.mn.emp.grad.year.10, echo=TRUE}
no.mn.emp.grad.year.10.model.postprune$variable.importance

importance <- no.mn.emp.grad.year.10.model.postprune$variable.importance
...
```{r compare accuracy no.mn.emp.grad.year.10, echo=TRUE}
data.frame(base_accuracy, accuracy_preprun, accuracy_postprun)
...
The pre-pruned tree provided better accuracy than the base model so let's use the most important variables from both.
...
-   A few that just don't make the cut either
    ...
    -   MCA.R
    -   grad.year.covid
-   Varaibles that are very important 5 and 10 years after graduating high school
    -   Dem_Desc
    -   ps.grad.InstitutionSector
    -   highest.cred.level
    -   ps.grad.location
-   Others that seem important
    -   range.school
    -   cte.achievement
    -   total.cte.courses.taken
    -   SpecialEdStatus
...
```{r create table for importance variables over time no MN emp}
  ...
  mutate(ind.var = as_factor(ind.var),
         var.group = ifelse(ind.var %in% c("grad.year.covid", "economic.status", "SpecialEdStatus", "RaceEthnicity", "english.learner"), "Demographics",
                            ifelse(ind.var %in% c("range.school", "Dem_Desc"), "HS Characteristics",
                                   ifelse(ind.var %in% c("pseo.participant", "took.ACT", "ACTCompositeScore", "MCA.M", "MCA.R", "MCA.S"), "Prep for PS",
                                          ifelse(ind.var %in% c("cte.achievement", "total.cte.courses.taken"), "CTE Engagement", "Post-secondary path")))),
         year.one = ifelse(ind.var %in% c("SpecialEdStatus", "took.ACT", "ACTCompositeScore", "range.school", "english.learner", "grad.year.covid"), "Important",
                           ifelse(ind.var %in% c("RaceEthnicity"), "Less so", "")),
         year.five = ifelse(ind.var %in% c("ps.grad.location", "ps.grad.InstitutionSector", "highest.cred.level", "ps.grad", "Dem_Desc"), "Important",
                            ifelse(ind.var %in% c("range.school", "cte.achievement", "pseo.participant", "total.cte.courses.taken", "MCA.M", "RaceEthnicity", "took.ACT", "ACTCompositeScore", "SpecialEdStatus", "MCA.R"), "Less so", "")),
         year.ten = ifelse(ind.var %in% c("ps.grad.location", "ps.grad.InstitutionSector", "highest.cred.level", "range.school", "Dem_Desc", "cte.achievement"), "Important",
                           ifelse(ind.var %in% c("total.cte.courses.taken", "took.ACT", "RaceEthnicity"), "Less so", "")))
