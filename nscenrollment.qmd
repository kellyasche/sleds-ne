...
Okay, lets check out the National Student Clearinghouse enrollment data. I will first import the entire dataset, clean it up, and then merge it with the master / Northeast graduate dataset.

<br>

```{r nsc enrollment originals}
nsc.enrollment.original <- read_csv('/Users/kellyasche/Library/CloudStorage/GoogleDrive-kasche@ruralmn.org/My Drive/Research/FY25-27/FY27/OHE Journey to meaningful employment/Data/SLEDS/NSCEnrollment/NSC_Enrollments.csv') %>%
...
6.  At any point during their college career, did the PersonID attend a college(s) inside or outside of the Northeast Region or outside the County of their high school?
...
```{r prep ipeds}
ipeds.original <- read_csv('/Users/kellyasche/Library/CloudStorage/GoogleDrive-kasche@ruralmn.org/My Drive/Research/FY25-27/FY27/OHE Journey to meaningful employment/Data/SLEDS/IPEDS/IPEDS.csv') 
...
One thing that's important is to realize that joining the nsc.enrollment dataset with the Northeast graduates dataset means we will have some NAs. Some of the students listed in the nsc.enrollment dataset aren't in the Northeast graduates dataset since they may have graduated before 2008 or didn't actually meet the criteria in the Northeast graduates dataset. Therefore, as we move forward, I will need to make sure to remove those NAs.
...
So there are fewer observations in this dataset than previous subsets. Why? This is due to dropping and PersonID that wasn't in the Northeast graduate dataset. I did not do that for the previous subsets. However, those previous subsets will be filtered down once we join it with the master dataset.
...
Up next we will create a dataset indicating whether a PersonID that graduated from a Northeast MN high school attended a post-secondary institution in their high school's EDR.
...
Next we will determine which Northeast MN graduates attended a post-secondary institution in the same planning region (Northeast planning region).
...
Below is the percentage of students that attended a post-secondary institution from the entire dataset. 68% of the PersonIDs in the dataset attended a post-secondary institution at some point between 2008 and 2024.
...
The table and distribution chart below show that a large majority (56.3%) of individuals attended only one post secondary institutions followed by 28% attending two.
...
The chart below provides the percentage of students in the entire dataset that attended each institution sector. By far, a huge majority attend either a public 4-year (32%) or public 2-year (53%). 15% attend a private, 4-year institution.

<br>

```{r total college types}
sectors <- read_xlsx('/Users/kellyasche/Library/CloudStorage/GoogleDrive-kasche@ruralmn.org/My Drive/Research/FY25-27/FY27/OHE Journey to meaningful employment/Data/SLEDS/NSC/IPEDS/institution-sectors.xlsx')
...
The chart below shows that nearly half (53.4%) of students graduating from a NE MN high school attended a post secondary institution that was located in a county with the same RUCA category as their high school.
...
The chart below shows that nearly 43% of students graduating from NE MN High schools end up attending a post-secondary institution in the same region.
...
The chart below shows that 70% of students graduating from Northeast MN high schools attend a post secondary institution in Minnesota.
...
The primary difference is that as districts become more urban, the percentage of graduates that attend a college in Minnesota declines - 70% in urban/town/rural mix districts compared to 74% in entirely rural districts.
...
Outside of Minnesota, Northeast MN graduates attend colleges in Wisconsin (9.2%), and North Dakota (5.4%) at the highest rate.
